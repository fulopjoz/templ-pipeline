...
sed -i "s/templ-pipeline\.dyn\.cloud\.e-infra\.cz/${FULL_DOMAIN}/g" k8s/ingress.yaml

# Update deployment image
sed -i "s|hub.cerit.io/\[your-username\]/templ-pipeline:latest|hub.cerit.io/${HARBOR_USERNAME}/templ-pipeline:latest|g" k8s/deployment.yaml

echo "✓ Configuration files updated"

echo ""
echo "Step 2: Building Docker image..."
./scripts/build.sh latest "$HARBOR_USERNAME"

echo ""
echo "Step 3: Pushing to Harbor registry..."
echo "Please ensure you are logged in to Harbor:"
echo "  docker login hub.cerit.io"
echo ""
...
echo "  ./scripts/build.sh v1.1.0 $HARBOR_USERNAME"
echo "  docker push hub.cerit.io/$HARBOR_USERNAME/templ-pipeline:v1.1.0"
echo "  kubectl set image deployment/templ-pipeline templ-pipeline=hub.cerit.io/$HARBOR_USERNAME/templ-pipeline:v1.1.0 -n $NAMESPACE"
echo ""
echo "======================================================="
