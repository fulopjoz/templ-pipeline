...
def get_pocket_chain_ids_from_file(pocket_file: str) -> List[str]:
    ...
    with open(pocket_file) as f:
        chains = sorted({line[21].strip() for line in f 
                         if line.startswith("ATOM") and len(line) > 21 and line[21].strip()})
    
    return chains

def find_pocket_chains(protein_path: str, ligand_path: str = None, distance_cutoff: float = 5.0) -> List[str]:
    ...
    if ligand_path is None or not os.path.exists(ligand_path):
        logger.warning(f"No ligand file found for {pdb_id}. Cannot identify pocket chains.")
        return []
    ...
    logger.info(f"Identified {len(pocket_chains)} pocket chains using distance cutoff {distance_cutoff}Å")
...
def find_pdbbind_paths(pdb_id: str, data_dir: str = "data") -> Dict[str, str]:
    ...
    for base_path in potential_base_paths:
        # Check refined set
        refined_dir = os.path.join(base_path, "PDBbind_v2020_refined", "refined-set", pdb_id)
        if os.path.exists(refined_dir):
            ...
            if os.path.exists(protein_path):
                ...
                logger.info(f"Found protein file for {pdb_id} in refined set at {protein_path}")
                return result
        
        # Check general set
        general_dir = os.path.join(base_path, "PDBbind_v2020_other_PL", "v2020-other-PL", pdb_id)
        if os.path.exists(general_dir):
            ...
            if os.path.exists(protein_path):
                ...
                logger.info(f"Found protein file for {pdb_id} in general set at {protein_path}")
                return result
    
    logger.warning(f"Could not find PDBbind files for {pdb_id} in any of the expected locations")
    return result
...
def load_sdf_molecules_cached(sdf_path: Path, cache: Dict = None, memory_limit_gb: float = 8.0) -> List[Any]:
    ...
    try:
        ...
        if initial_memory_gb > memory_limit_gb:
            logger.warning(f"Process already using {initial_memory_gb:.1f}GB, above limit {memory_limit_gb}GB")
    except ImportError:
        initial_memory_gb = 0
    ...
    try:
                        ...
                        if processed_count % 1000 == 0:
                            try:
                                ...
                                if memory_delta > memory_limit_gb:
                                    logger.warning(f"Memory limit exceeded at {processed_count} molecules, stopping")
                                    break
        ...
        logger.info(f"Loaded {len(molecules)} molecules from {sdf_path.name} (skipped {skipped_count})")
...
def find_ligand_by_pdb_id(pdb_id: str, molecules: List[Any]) -> Tuple[Optional[str], Any]:
...
def calculate_rmsd(pose_mol: Any, crystal_mol: Any) -> float:
    ...
    try:
        ...
        if rdShapeAlign is not None:
            try:
                rdShapeAlign.AlignMol(ref, pose, refConfId=0, probeConfId=0, useColors=True)
            except Exception:
...
def get_protein_file_paths(pdb_id: str, data_dir: Path) -> List[Path]:
    ...
    return [
        data_dir / "PDBBind" / "PDBbind_v2020_refined" / "refined-set" / pdb_id / f"{pdb_id}_protein.pdb",
        data_dir / "PDBBind" / "PDBbind_v2020_other_PL" / "v2020-other-PL" / pdb_id / f"{pdb_id}_protein.pdb",
        data_dir / "PDBbind_v2020_refined" / "refined-set" / pdb_id / f"{pdb_id}_protein.pdb",
        data_dir / "PDBbind_v2020_other_PL" / "v2020-other-PL" / pdb_id / f"{pdb_id}_protein.pdb"
    ]
    ...
    for embedding_file in embedding_files:
        if embedding_file.exists():
            try:
                ...
                logger.info(f"Loaded {len(pdbs)} PDBs from {split_file}, {len(filtered_pdbs)} have embeddings")
                return filtered_pdbs
            except Exception as e:
                logger.warning(f"Could not filter by embeddings from {embedding_file}: {e}")
                break
