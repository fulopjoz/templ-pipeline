...
def get_optimized_worker_config(workload_type: str = "balanced", dataset_size: int = 0) -> dict:
...
def get_hardware_info() -> dict:
    ...
    return {
        ...
        "total_memory_gb": f"{mem_total_gb:.1f}" if isinstance(mem_total_gb, float) else mem_total_gb,
        "available_memory_gb": f"{mem_free_gb:.1f}" if isinstance(mem_free_gb, float) else mem_free_gb,
