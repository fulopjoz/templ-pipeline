...
class CoordinateMapper:
    ...
    def map_heavy_atoms(mol_before: Chem.Mol, mol_after: Chem.Mol) -> Dict[int, int]:
        ...
        while before_idx < mol_before.GetNumAtoms() and after_idx < mol_after.GetNumAtoms():
    ...
    @staticmethod
    def preserve_heavy_atom_coords(mol_source: Chem.Mol, mol_target: Chem.Mol) -> Chem.Mol:
...
class FixedMolecularProcessor:
    ...
    def create_independent_copy(mol: Chem.Mol) -> Chem.Mol:
        ...
        if not isinstance(mol, Chem.Mol):
            logger.error(f"Invalid input type for molecular copy: {type(mol)}, expected Chem.Mol")
            return None
        ...
        try:
            ...
            logger.warning(f"Primary molecular copy failed: {e}, trying fallback methods")
        ...
        if mol.GetNumConformers() > 0:
            ...
            if valid_conformers_added == 0:
                logger.warning("No valid conformers could be copied, embedding new conformer")
                try:
    ...
    def safe_add_hydrogens(mol: Chem.Mol, preserve_coords: bool = True) -> Chem.Mol:
        ...
        if preserve_coords and FixedMolecularProcessor.validate_conformer_access(mol_copy):
        ...
        try:
            ...
            logger.warning(f"AddHs with coordinates failed: {e}, trying without coordinates")
            try:
        ...
        if preserve_coords and original_heavy_coords and FixedMolecularProcessor.validate_conformer_access(mol_with_h):
            try:
                ...
                for new_idx in range(mol_with_h.GetNumAtoms()):
                    ...
                    while orig_heavy_idx in original_heavy_coords:
                        ...
                        if (new_atom.GetSymbol() == orig_data['symbol'] and 
                            new_atom.GetFormalCharge() == orig_data['charge']):
                ...
                for orig_idx, new_idx in heavy_atom_mapping.items():
                    if orig_idx in original_heavy_coords:
                        try:
                            conf.SetAtomPosition(new_idx, original_heavy_coords[orig_idx]['pos'])
                        except Exception as e:
                            logger.warning(f"Failed to set position for atom {new_idx}: {e}")
            except Exception as e:
    ...
    @staticmethod
    def add_hydrogens_with_fallback(mol: Chem.Mol, preserve_coords: bool = True) -> Chem.Mol:
...
def validate_molecular_geometry(mol: Chem.Mol, mol_name: str = "molecule") -> Tuple[bool, str]:
    """
    ...
    try:
        ...
        if max_coord > 1000.0:  # Reasonable molecules should have coordinates < 1000 Å
            return False, f"{mol_name}: Suspiciously large coordinates (max: {max_coord:.2f})"
        ...
        if num_atoms > 1:
            ...
            if max_distance < 0.1:  # All atoms within 0.1 Å suggests collapse
                return False, f"{mol_name}: All atoms collapsed to same position (max_dist: {max_distance:.3f})"
        
        # Check for unrealistic bond lengths (hydrogens too far from heavy atoms)
        heavy_atoms = [i for i in range(num_atoms) if mol.GetAtomWithIdx(i).GetAtomicNum() != 1]
        hydrogen_atoms = [i for i in range(num_atoms) if mol.GetAtomWithIdx(i).GetAtomicNum() == 1]
        ...
        if heavy_atoms and hydrogen_atoms:
            ...
            for h_idx in hydrogen_atoms:
                ...
                if min_dist_to_heavy > 3.0:  # 3.0 Å is very generous for H-heavy atom distance
                    suspicious_hydrogens += 1
            
            # If more than 20% of hydrogens are suspiciously placed, flag as corrupt
            if suspicious_hydrogens > 0.2 * len(hydrogen_atoms) and suspicious_hydrogens > 2:
                return False, f"{mol_name}: {suspicious_hydrogens}/{len(hydrogen_atoms)} hydrogens at suspicious distances"
    ...
    try:
        ...
        if mol.GetNumConformers() > 0:
            ...
            try:
                ...
                if extreme_coords:
                    logger.debug(f"Extreme coordinates found: {extreme_coords[:5]} (showing first 5)")
                    return False, f"Conformer has extreme coordinates (max: {max(extreme_coords):.1f})"
...
def score_and_align(conf: Chem.Mol, tpl: Chem.Mol) -> Tuple[Dict[str, float], Chem.Mol]:
    ...
    try:
        tpl_processed, tpl_had_metals, tpl_subs = detect_and_substitute_organometallic(template_copy, "template")
        prb_processed, prb_had_metals, prb_subs = detect_and_substitute_organometallic(probe_copy, "probe")
    except Exception as e:
        logger.warning(f"Organometallic processing failed: {e}, using original molecules")
    ...
    try:
        ...
        prb_with_h_for_scoring = FixedMolecularProcessor.safe_add_hydrogens(prb_heavy_only, preserve_coords=True)
        tpl_with_h_for_scoring = FixedMolecularProcessor.safe_add_hydrogens(tpl_heavy_only, preserve_coords=True)
        
        # Validate molecules after hydrogen addition
        if not FixedMolecularProcessor.validate_conformer_access(prb_with_h_for_scoring):
            logger.warning("Probe invalid after hydrogen addition")
            prb_with_h_for_scoring = prb_processed
        
        if not FixedMolecularProcessor.validate_conformer_access(tpl_with_h_for_scoring):
        ...
        try:
            ...
            if isinstance(alignment_result, tuple) and len(alignment_result) == 2:
                ...
                raise Exception(f"Unexpected alignment result type: {type(alignment_result)}")
            ...
            try:
                ...
                raise Exception(f"Alignment returned non-numeric scores: {shape_score}, {color_score} - {e}")
                
        except Exception as e:
            logger.warning(f"Heavy-atom alignment failed: {e}, falling back to original method")
        ...
        try:
            ...
            final_aligned_mol = FixedMolecularProcessor.safe_add_hydrogens(aligned_heavy_only, preserve_coords=False)
            ...
            logger.debug(f"Heavy-atom alignment completed: shape={shape_score:.3f}, color={color_score:.3f}, combo={combo_score:.3f}")
            
            # Validate the final aligned molecule geometry
            is_valid, validation_msg = validate_molecular_geometry(final_aligned_mol, "aligned_probe")
            if not is_valid:
            ...
            return {"shape": float(shape_score), "color": float(color_score), "combo": 0.5*(float(shape_score)+float(color_score))}, prb_heavy_only
        
    except Exception as e:
        logger.warning(f"Heavy-atom alignment pipeline failed: {e}, falling back to original method")
        return _fallback_alignment(prb_processed, tpl_processed)

def _fallback_alignment(prb_processed: Chem.Mol, tpl_processed: Chem.Mol) -> Tuple[Dict[str, float], Chem.Mol]:
    """Fallback alignment method with robust error handling."""
    try:
        prb_with_h = FixedMolecularProcessor.safe_add_hydrogens(prb_processed, preserve_coords=True)
        tpl_with_h = FixedMolecularProcessor.safe_add_hydrogens(tpl_processed, preserve_coords=True)
    except Exception as e:
    ...
    try:
        ...
        if isinstance(alignment_result, tuple) and len(alignment_result) == 2:
            ...
            raise Exception(f"Unexpected fallback alignment result type: {type(alignment_result)}")
        ...
        try:
            ...
            raise Exception(f"Fallback alignment returned non-numeric scores: {shape_score}, {color_score} - {e}")
        ...
        logger.warning("Using fallback alignment method - poses may have hydrogen geometry issues")
...
def _score_and_align_task(args_tuple):
    ...
    try:
        ...
        if not isinstance(single_conf_mol, Chem.Mol) or not isinstance(tpl_mol, Chem.Mol):
            logger.error(f"Invalid molecule types for conformer {original_cid}: {type(single_conf_mol)}, {type(tpl_mol)}")
            return None
        ...
        if no_realign_flag_for_pose_selection:
            ...
            pose_to_consider_if_best = aligned_mol if aligned_mol is not None else single_conf_mol
        
        # Final validation of output molecule
        if pose_to_consider_if_best is None or not isinstance(pose_to_consider_if_best, Chem.Mol):
            logger.warning(f"Invalid output molecule for conformer {original_cid}, using input molecule")
            pose_to_consider_if_best = single_conf_mol
        ...
        logger.error(f"Error in _score_and_align_task for conformer {original_cid if 'original_cid' in locals() else 'unknown'}: {e}")
        import traceback
...
def select_best(
    ...
    for batch_start in range(0, n_confs, batch_size):
        ...
        logger.debug(f"Processing batch {batch_start//batch_size + 1}: conformers {batch_start}-{batch_end-1}")
        ...
        for conf_id in batch_conf_ids:
            try:
                ...
                if conf_id >= confs.GetNumConformers():
                    logger.warning(f"Conformer {conf_id} doesn't exist (total: {confs.GetNumConformers()})")
                    continue
                ...
                if conf_mol is None:
                    logger.warning(f"Failed to copy molecule for conformer {conf_id}, trying direct approach")
                    ...
                    try:
                        ...
                        logger.warning(f"SMILES fallback failed for conformer {conf_id}: {smiles_err}")
                        continue
                ...
                try:
                    ...
                    if temp_mol.GetNumConformers() == 0:
                        logger.warning(f"No conformers after processing conformer {conf_id}")
                        continue
                    
                    args_list.append((temp_mol, tpl, conf_id, no_realign))
                    
                except Exception as conf_err:
                    logger.warning(f"Conformer processing failed for {conf_id}: {conf_err}")
                    continue
                
            except Exception as e:
                logger.warning(f"Failed to prepare conformer {conf_id}: {e}")
                import traceback
                logger.debug(f"Conformer preparation traceback: {traceback.format_exc()}")
                continue
        
        # Process this batch
        if n_workers > 1 and len(args_list) > 1:
            with ProcessPoolExecutor(max_workers=min(n_workers, len(args_list))) as executor:
            ...
            if result is not None:
                try:
                    ...
                    if len(result) == 3:
                        ...
                        # Validate scores
                        if isinstance(scores, dict) and all(metric in scores for metric in ['shape', 'color', 'combo']):
                            # Validate molecule
                            if mol is not None and isinstance(mol, Chem.Mol):
                                all_results.append(result)
                            else:
                                logger.warning(f"Invalid molecule in result for conformer {conf_id}")
                        else:
                            logger.warning(f"Invalid scores in result for conformer {conf_id}: {scores}")
                    else:
    ...
    for metric in ['shape', 'color', 'combo']:
        ...
        if metric_results:
            ...
            best_conf_id, best_scores, best_mol = metric_results[0]  # Fixed unpacking order
            ...
            try:
                ...
                if not isinstance(best_mol, Chem.Mol):
                    logger.error(f"Best molecule for {metric} has invalid type: {type(best_mol)}")
                    continue
                
                output_mol = FixedMolecularProcessor.create_independent_copy(best_mol)
                
                if output_mol is None:
                    logger.warning(f"Failed to create copy of best {metric} pose, trying direct assignment")
                    # Fallback: use original molecule if copying fails
                    output_mol = best_mol
                
                best_poses[metric] = (output_mol, best_scores)
                logger.debug(f"Best {metric}: conf {best_conf_id}, score {best_scores[metric]:.3f}")
    ...
    logger.info(f"Selected {len(best_poses)} best poses from {len(all_results)} scored conformers")
    return best_poses
...
def generate_properties_for_sdf(mol: Chem.Mol, metric: str, score: float, template_pid: str, 
                              template_info: Dict[str, str] = None) -> Chem.Mol:
