...
class SimpleTimeSplitRunner:
    ...
    def get_allowed_templates_for_split(
    ...
    def run_single_target_subprocess(
        self,
        target_pdb: str,
        ...
        try:
            ...
            ligand_smiles, crystal_mol = self.molecule_loader.get_ligand_data(
                target_pdb
            )
            if not ligand_smiles:
                ...
                if self.skip_tracker and processing_stage in [
                    "pre_pipeline_excluded",
                    "pipeline_filtered",
                ]:
                    self.skip_tracker.track_skip(
                        target_pdb, "ligand_data_missing", error_msg
                    )
                ...
                if crystal_mol is not None:
                    ...
                    if is_large:
                        ...
                        reason_key = (
                            ...
                            if "polysaccharide" in error_msg.lower()
                            or "saccharide" in error_msg.lower()
                            else "large_peptide"
                        )
                        ...
                        if self.skip_tracker:
                            self.skip_tracker.track_skip(
                                target_pdb, reason_key, error_msg
                            )
            ...
            cmd = [
                "templ",
                "--output-dir",
                str(self.results_dir / "poses" / target_pdb.lower()),
                "run",
                "--protein-pdb-id",
                target_pdb,
                "--ligand-smiles",
            ...
            if result.returncode == 0:
                ...
                if self.skip_tracker and processing_stage in [
                    ...
                    self.skip_tracker.track_skip(
                        target_pdb, "cli_execution_failed", error_msg
                    )
            ...
            if self.skip_tracker and processing_stage in [
                ...
                self.skip_tracker.track_skip(target_pdb, "pipeline_error", error_msg)
    ...
    def run_split_benchmark(
        ...
        logger.info(
            f"Running {split_name} split with {len(target_pdbs)} targets using {n_workers} workers"
        )
        ...
        for batch_idx in range(total_batches):
            ...
            with ProcessPoolExecutor(max_workers=n_workers) as executor:
                ...
                for target_pdb in batch_pdbs:
                    ...
                    allowed_templates = self.get_allowed_templates_for_split(
                        target_split, target_pdb
                    )
                ...
                for future in as_completed(future_to_pdb):
                    ...
                    try:
                        ...
                        affects_success_rate = result.get(
                            "affects_pipeline_success_rate", True
                        )
                        ...
                        processing_stage = error_result.get(
                            "processing_stage", "pipeline_attempted"
                        )
                        if processing_stage == "pre_pipeline_excluded":
                            pre_pipeline_excluded_count += 1
                    ...
                    pipeline_success_rate = (
                        (pipeline_success_count / pipeline_attempted_count * 100)
                        if pipeline_attempted_count > 0
                        else 0
                    )
        ...
        data_completeness_rate = (
            ((processed_count - pre_pipeline_excluded_count) / processed_count * 100)
            if processed_count > 0
            else 0
        )
        molecule_acceptance_rate = (
            (
                pipeline_attempted_count
                / (processed_count - pre_pipeline_excluded_count)
                * 100
            )
            if (processed_count - pre_pipeline_excluded_count) > 0
            else 0
        )
        ...
        try:
            ...
            if self.skip_tracker:
                ...
                if skip_stats["total_skipped"] > 0:
                    ...
                    skip_summary_file = self.skip_tracker.save_summary(
                        f"skip_summary_{split_name}_{self.benchmark_timestamp}.json"
                    )
    ...
    def _generate_detailed_summaries(
        self, results_jsonl: Path, split_name: str, timestamp: str
    ):
        """Generate detailed 2A/5A success rate summaries using BenchmarkSummaryGenerator."""
        try:
            logger.info(
                f"Generating detailed 2A/5A success rate summaries for {split_name} split..."
            )
            ...
            detailed_summary_file = (
                summaries_dir / f"detailed_summary_{split_name}_{timestamp}.json"
            )
            ...
            try:
                ...
                if csv_summary is not None and hasattr(csv_summary, "to_csv"):
                    csv_file = (
                        summaries_dir
                        / f"benchmark_summary_{split_name}_{timestamp}.csv"
                    )
            ...
            if isinstance(unified_summary, dict) and "summary" in unified_summary:
                for row in unified_summary["summary"]:
                    ...
                    logger.info(
                        f"✓ 2A Success Rate: {success_rate_2A} (from {pipeline_attempted} attempted)"
                    )
                    logger.info(
                        f"✓ 5A Success Rate: {success_rate_5A} (from {pipeline_attempted} attempted)"
                    )

            logger.info(
                f"✓ Detailed summaries with 2A/5A success rates saved to: {summaries_dir}"
            )
    ...
    def cleanup(self):
        ...
        try:
            ...
            if self.shared_embedding_cache:
                ...
                logger.info(
                    f"Cleaning up shared embedding cache: {self.shared_embedding_cache}"
                )
    ...
    def _precompute_template_sets(self):
        ...
        logger.info(
            f"  Val allowed templates: {len(self._val_allowed_templates)} (train only)"
        )
        logger.info(
            f"  Train allowed templates: {len(self._train_allowed_templates)} (train only, LOO applied)"
        )
...
def main():
    ...
    parser.add_argument(
        "--n-conformers", type=int, default=200, help="Number of conformers"
    )
    parser.add_argument(
        "--max-pdbs", type=int, help="Max PDBs to process (for testing)"
    )
    parser.add_argument("--timeout", type=int, default=600, help="Timeout per target")
    parser.add_argument(
        "--use-enhanced-shared-data",
        action="store_true",
        help="Use enhanced shared data manager",
    )
