...
class BenchmarkSummaryGenerator:
    ...
    def _generate_polaris_summary(self, results_data: Dict, output_format: str) -> Any:
        ...
        for result_key, virus_type, dataset, template_source in result_configs:
            if result_key in results_section:
                ...
                query_count = result_entry.get(
                    "query_count", len(result_entry.get("results", {}))
                )
                template_counts = result_entry.get("template_counts", {})

                # Format template description
                template_desc = self._format_template_description(
                    template_counts, dataset, query_count
                )
    ...
    def _validate_rmsd_calculation(self, split_results: List[Dict]) -> Dict:
        ...
        for result in split_results:
            if result.get("success"):
                ...
                if has_valid_rmsd:
                    ...
                    validation_report["metrics_with_rmsd_failures"].extend(
                        metrics_with_failures
                    )
        ...
        if validation_report["results_with_null_rmsd"] > 0:
            failure_rate = (
                validation_report["results_with_null_rmsd"]
    ...
    def _parse_exclusion_reason(self, error_msg: str) -> str:
        ...
        elif "peptide" in error_msg_lower and (
            "residues" in error_msg_lower or "threshold" in error_msg_lower
        ):
            return "large_peptide"
        ...
        elif "could not load ligand smiles" in error_msg_lower or (
        ...
        # Parse pipeline processing exclusions
            ...
            "no poses generated" in error_msg_lower
            or "pose generation failed" in error_msg_lower
        ):
            return "pose_generation_failed"
        elif (
            "rmsd calculation failed" in error_msg_lower
            or "rmsd failed" in error_msg_lower
        ):
            return "rmsd_calculation_failed"
        elif (
            "conformer generation failed" in error_msg_lower
            or "conformer failed" in error_msg_lower
        ):
            return "conformer_generation_failed"
        elif (
            "alignment failed" in error_msg_lower
            or "molecular alignment" in error_msg_lower
        ):
        ...
        elif "mcs" in error_msg_lower and (
            "failed" in error_msg_lower or "calculation" in error_msg_lower
        ):
            ...
            "embedding failed" in error_msg_lower
            or "embedding error" in error_msg_lower
        ):
            return "embedding_failed"
        elif "memory" in error_msg_lower and (
            ...
            "very close constraints" in error_msg_lower
            or "very distant constraints" in error_msg_lower
        ):
    ...
    def _classify_exclusion_processing_stage(self, exclusion_reason: str) -> str:
        ...
        pre_pipeline_exclusions = {
            "ligand_data_missing",
            "protein_file_missing",
            "template_file_missing",
            "embedding_file_missing",
            "crystal_structure_missing",
            "file_not_found",
            "cli_command_invalid",
            "subprocess_failed",
        ...
        pipeline_filter_exclusions = {
            "large_peptide",
            "rhenium_complex",
            "complex_polysaccharide",
            "molecule_validation_failed",
            "invalid_molecule",
            "molecule_too_large",
            "molecule_too_small",
            "poor_quality_crystal",
            "invalid_smiles",
            "validation_failed",
            "geometry_validation_failed",
            "molecule_sanitization_failed",
            "quality_filter",
            "molecule_filtered",
        ...
        pipeline_execution_failures = {
            ...
            "rmsd_calculation_failed",
            "conformer_generation_failed",
            "molecular_alignment_failed",
            "mcs_calculation_failed",
            "central_atom_embedding_failed",
            "constrained_embedding_failed",
            "embedding_failed",
            "molecular_connectivity_failed",
            "molecular_optimization_failed",
            "force_field_failed",
            "coordinate_access_failed",
            "invalid_coordinates",
            "molecular_fragmentation",
            "invalid_atom_positions",
            "suspicious_bond_lengths",
            "empty_coordinate_map",
            "constraint_distance_issues",
            "minimization_no_conformers",
            "rascal_mcs_failed",
            "mcs_too_small",
            "rascal_search_failed",
            "invalid_smarts_pattern",
            "invalid_mcs_match",
            "hydrogen_addition_failed",
            "mcs_hydrogen_addition_failed",
            "mcs_index_mismatch",
            "mcs_matching_inconsistency",
            "atom_mapping_failed",
            "insufficient_coordinate_constraints",
            "relaxed_constraints_failed",
            "progressive_embedding_failed",
            "zero_conformers_generated",
            "molecular_distortion",
            "alignment_index_mismatch",
            "rdkit_alignment_failed",
            "alignment_skipped",
            "connectivity_issues",
            "organometallic_embedding_failed",
            "all_embedding_methods_failed",
            "uff_fallback_failed",
            "mmff_parameter_validation_failed",
            "memory_error",
            "pipeline_error",
            "algorithm_failure",
            "processing_failed",
        ...
        if exclusion_reason in pre_pipeline_exclusions:
            ...
            if any(
                pattern in exclusion_lower
                for pattern in ["missing", "not found", "file", "data"]
            ):
                return "pre_pipeline_excluded"
            elif any(
                pattern in exclusion_lower
                for pattern in ["validation", "filter", "quality", "invalid"]
            ):
                return "pipeline_filtered"
            elif any(
                pattern in exclusion_lower
                for pattern in ["timeout", "failed", "error", "exception"]
            ):
                return "pipeline_attempted"
            else:
                ...
                logger.warning(
                    f"Unclassified error pattern: {exclusion_reason} - treating as pipeline_attempted"
                )
                return "pipeline_attempted"
    ...
    def _classify_no_templates_case(self, result: Dict) -> str:
        ...
        try:
            ...
            if json_start != -1:
                ...
                requested_templates = cli_result.get("template_filtering_info", {}).get(
                    "requested_templates", 0
                )
                found_templates = cli_result.get("template_filtering_info", {}).get(
                    "found_templates", 0
                )

                logger.debug(
                    f"CLI JSON parsed: total_db={total_templates_in_db}, found={found_templates}, requested={requested_templates}"
                )
            ...
            logger.debug(
                f"Failed to parse CLI JSON output for no-templates classification: {e}"
            )
            return "no_templates_found"
    ...
    def _calculate_timesplit_metrics_with_exclusions(
        self,
        ...
        metrics = self._calculate_timesplit_metrics(
            successful_results, total_targets, all_results
        )
        ...
        if exclusion_breakdown:
            ...
            for reason, count in sorted(
                ...
                logger.info(
                    f"EXCLUSION_ANALYSIS:   {reason}: {count} ({percentage:.1f}%)"
                )

        return metrics, exclusion_stats

    def _calculate_timesplit_metrics(
        self,
        ...
        if all_results:
                ...
                if pipeline_stage in stage_counts:
                    ...
                    if processing_stage in stage_counts:
                        ...
                        if result.get("success") and result.get("rmsd_values"):
                            ...
                            error_msg = result.get("error", "")
                            exclusion_reason = self._parse_exclusion_reason(error_msg)
                            fallback_stage = self._classify_exclusion_processing_stage(
                                exclusion_reason
                            )
            ...
            logger.info(
                f"SUCCESS_RATE_CALC:   Pipeline attempted: {pipeline_attempted_targets}"
            )
        ...
        logger.info(
            f"SUCCESS_RATE_CALC:   Pipeline attempted targets (denominator): {pipeline_attempted_targets}"
        )
        ...
        for result in results_to_process:
            ...
            if result.get("success") and result.get("rmsd_values"):
                ...
                logger.info(
                    f"SUCCESS_RATE_CALC: Including pipeline failure as 999.0Å RMSD: {error_msg[:100]}..."
                )
        ...
        logger.info(
            f"SUCCESS_RATE_CALC:   Total RMSD entries (including failures): {len(rmsd_by_metric.get('combo', []))}"
        )
        ...
        logger.info(
            f"DATASET_SIZE_REPORTING:   Pre-pipeline excluded: {pre_pipeline_excluded} (missing files, invalid data)"
        )
        logger.info(
            f"DATASET_SIZE_REPORTING:   Pipeline filtered: {pipeline_filtered} (peptides, validation failures)"
        )
        logger.info(
        ...
        for metric_key in rmsd_by_metric:
            ...
            logger.info(
                f"SUCCESS_RATE_CALC:   {metric_key}: {rmsd_count} RMSD values collected"
            )

        # Calculate statistics for each metric
        for metric_key in set(
            list(rmsd_by_metric.keys()) + list(score_by_metric.keys())
        ):
            ...
            if rmsds:
                ...
                rate_2A = (
                    count_2A / pipeline_attempted_targets * 100
                    if pipeline_attempted_targets > 0
                    else 0
                )
                rate_5A = (
                    count_5A / pipeline_attempted_targets * 100
                    if pipeline_attempted_targets > 0
                    else 0
                )
                ...
                legacy_rate_2A = (
                    count_2A / total_targets * 100 if total_targets > 0 else 0
                )
                legacy_rate_5A = (
                    count_5A / total_targets * 100 if total_targets > 0 else 0
                )
                ...
                logger.info(
                    f"SUCCESS_RATE_CALC: *** FINAL SUCCESS RATES FOR {metric_key.upper()} ***"
                )
                logger.info(
                    f"SUCCESS_RATE_CALC:   2A pipeline success: {rate_2A:.1f}% ({count_2A}/{pipeline_attempted_targets}) [CORRECTED - USE FOR PAPERS]"
                )
                logger.info(
                    f"SUCCESS_RATE_CALC:   5A pipeline success: {rate_5A:.1f}% ({count_5A}/{pipeline_attempted_targets}) [CORRECTED - USE FOR PAPERS]"
                )
                logger.info(
                    f"SUCCESS_RATE_CALC:   Includes {pipeline_failures} pipeline failures as 0 successes (timeouts, MCS failures, etc.)"
                )
                logger.info(
                    f"SUCCESS_RATE_CALC:   Legacy 2A rate: {legacy_rate_2A:.1f}% ({count_2A}/{total_targets}) [BIASED - DON'T USE]"
                )
                logger.info(
                    f"SUCCESS_RATE_CALC:   Legacy 5A rate: {legacy_rate_5A:.1f}% ({count_5A}/{total_targets}) [BIASED - DON'T USE]"
                )
                logger.info(
                ...
                metrics[metric_key] = {
                    ...
                    "median_score": np.median(
                        scores
                    ),  # Report alignment scores separately
                    "error": "RMSD_CALCULATION_FAILED",
                }

                logger.warning(
                    f"SUCCESS_RATE_CALC: Benchmark results for {metric_key} are INVALID due to missing RMSD data"
                )
                logger.warning(
    ...
    def _validate_data_consistency(
        self, raw_results_file: Path, summary_data: Dict
    ) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            for split_name, split_data in raw_split_results.items():
                ...
                raw_pipeline_success_rate = split_data.get("success_rates", {}).get(
            ...
            if (
                abs(raw_pipeline_success_rate - summary_success_rate) > 5.0
            ):  # 5% tolerance
                validation_report["is_consistent"] = False
                validation_report["discrepancies"].append(
                    {
            ...
            logger.info(
                f"Data consistency validation: {'PASS' if validation_report['is_consistent'] else 'FAIL'}"
            )
            if not validation_report["is_consistent"]:
                logger.warning(
                    f"Found {len(validation_report['discrepancies'])} discrepancies"
                )
                for disc in validation_report["discrepancies"]:
                    logger.warning(
                        f"  {disc['metric']}: raw={disc['raw_value']}, summary={disc['summary_value']}"
                    )

        except Exception as e:
            validation_report["is_consistent"] = False
            validation_report["discrepancies"].append(
                {"metric": "validation_error", "error": str(e)}
            )
            logger.error(f"Data consistency validation failed: {e}")

        return validation_report

    def _generate_timesplit_summary_fixed(
        ...
        logger.info(
            f"Processing {len(results_data)} individual results for timesplit summary"
        )

        # Use stage-aware metrics calculation
        total_targets = len(results_data)
        successful_results = [
            r for r in results_data if r.get("success") and r.get("rmsd_values")
        ]
        ...
        metrics = self._calculate_timesplit_metrics(
            successful_results, total_targets, results_data
        )
        ...
        for metric_key, metric_data in metrics.items():
            ...
            if metric_key in ["shape", "color", "combo"]:
                ...
                summary_row = {
                    ...
                    "Pipeline_Failed": pipeline_attempted_targets
                    - len(successful_results),
                    "Pre_Pipeline_Excluded": pre_pipeline_excluded,
                    "Pipeline_Filtered": pipeline_filtered,
                    # FILTERING BREAKDOWN
                    "Peptide_Polysaccharide_Filtered": self._count_peptide_filtering(
                        results_data
                    ),
                    "Template_Database_Filtering": self._format_template_stats_for_display(
                        self._extract_template_filtering_stats(results_data)
                    ),
                    "Template_Database_Filtering_Details": self._extract_template_filtering_stats(
                        results_data
                    ),
        ...
        enhanced_summary = {
            ...
            "stage_aware_metrics": {
                ...
                "pipeline_success_rate": (
                    (len(successful_results) / pipeline_attempted_targets * 100)
                    if pipeline_attempted_targets > 0
                    else 0
                ),
            },
        ...
        logger.info(
            f"Stage-aware pipeline success rate: {enhanced_summary['stage_aware_metrics']['pipeline_success_rate']:.1f}%"
        )
    ...
    def _extract_template_filtering_stats(self, results: List[Dict]) -> Dict[str, Any]:
        ...
        for result in results:
            ...
            if "TEMPL_JSON_RESULT:" in stdout:
                try:
                    json_start = stdout.find("TEMPL_JSON_RESULT:") + len(
                        "TEMPL_JSON_RESULT:"
                    )
                    json_end = stdout.find("\n", json_start)
                    if json_end == -1:
                    ...
                    if total_templates > 0:
                        ...
                        template_db_stats = cli_result.get(
                            "template_database_stats", {}
                        )
                        if (
                            template_db_stats.get("filtered_peptides", 0) > 0
                            or template_db_stats.get("filtered_polysaccharides", 0) > 0
                        ):
        ...
        if total_templates_values:
            ...
            template_stats["avg_total_templates"] = template_stats["database_size"][
                "avg"
            ]
            template_stats["avg_used_templates"] = template_stats[
                "templates_used_stats"
            ]["avg"]
        else:
            template_stats = {
    ...
    def _format_template_description(
        self, template_counts: Dict, dataset: str, query_count: int
    ) -> str:
    ...
    def _generate_generic_summary(self, results_data: Dict, output_format: str) -> Any:
        ...
        summary = {
            ...
            "entry_types": (
                list(results_data.keys()) if isinstance(results_data, dict) else []
            ),
        ...
        table_data = [
            {
                ...
                "Keys": str(summary["entry_types"][:5])
                + ("..." if len(summary["entry_types"]) > 5 else ""),
            }
        ]
            ...
            try:
                if fmt == "csv" and df is not None:
                    ...
                    if hasattr(self, "_original_data_for_json"):
                        ...
                        PANDAS_AVAILABLE
                        and pd is not None
                        and isinstance(summary_data, pd.DataFrame)
                    ):
                    ...
                    enhanced_data = {"timestamp": timestamp, "summary": data_to_save}
                            ...
                            if isinstance(entry, dict):
                                ...
                                if exclusion_reasons:
                                    for reason, count in exclusion_reasons.items():
                                        total_exclusions[reason] = (
                                            total_exclusions.get(reason, 0) + count
                                        )
                        if total_exclusions:
                            enhanced_data["exclusion_summary"] = {
                                ...
                                "success_rate": (
                                    (total_with_rmsd / total_targets * 100)
                                    if total_targets > 0
                                    else 0
                                ),
                            }
...
def generate_summary_from_files(
    ...
    return generator.save_summary_files(
        summary, output_dir, "combined_benchmark_summary"
    )
...
if __name__ == "__main__":
    ...
    try:
        saved_files = generate_summary_from_files(
            all_files, args.output_dir, args.benchmark_type
        )
