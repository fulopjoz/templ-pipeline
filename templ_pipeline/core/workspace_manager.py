...
class WorkspaceManager:
    ...
    def _cleanup_all_registered(cls) -> None:
        ...
        logger.debug(f"Cleaning up {len(cls._registered_workspaces)} registered workspaces")
        ...
        for workspace in list(cls._registered_workspaces):
            try:
                ...
                logger.warning(f"Failed to cleanup registered workspace {workspace}: {e}")
...
class TempDirectoryManager(WorkspaceManager):
    ...
    def __init__(self, prefix: str = "templ_temp_", **kwargs):
        ...
        config = WorkspaceConfig(
            base_dir=str(temp_path.parent),
            auto_cleanup=True,
            lazy_creation=False
        )
        
        super().__init__(
            run_id=temp_path.name,
            config=config,
            base_name="",
            **kwargs
        )
