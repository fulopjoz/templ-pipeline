...
try:
    from templ_pipeline.core.utils import find_pdbbind_paths  # For finding PDB locations
except ImportError:
...
def _get_standard_embedding_paths() -> List[Path]:
    ...
    root_dir_candidate2 = current_dir.parent.parent.parent # If nested deeper, e.g. in 'src'
    ...
    if (root_dir_candidate2 / project_root_marker).exists() or \
       (root_dir_candidate2 / "templ_pipeline").exists() or \
       (root_dir_candidate2 / ".git").exists():
        root_dir = root_dir_candidate2
    elif not ((root_dir_candidate1 / "data").exists() or \
              (root_dir_candidate1 / "templ_pipeline").exists() or \
              (root_dir_candidate1 / ".git").exists() or \
              (root_dir_candidate1 / project_root_marker).exists()):
    ...
    potential_paths = [
        ...
        root_dir / "templ_pipeline" / "data" / "embeddings" / "protein_embeddings_base.npz",
        root_dir / "mcs_bench" / "data" / "protein_embeddings_base.npz",
        root_dir / "mcs_bench" / "data" / "esm2_embeddings" / "protein_embeddings_base.npz",
        Path.home() / ".cache" / "templ" / "embeddings" / "embeddings.npz", # Symlink
        Path("data", "embeddings", "protein_embeddings_base.npz"), # Relative to CWD
        Path("templ_pipeline", "data", "embeddings", "protein_embeddings_base.npz"), # Relative to CWD
        Path("/home/ubuntu/mcs/templ_pipeline/data/embeddings/protein_embeddings_base.npz") # Specific problematic path
    ]
    ...
    for p in potential_paths:
        ...
        except FileNotFoundError: # Path.resolve() can raise this if a component doesn't exist
            if p.is_absolute() and str(p) not in seen_paths_str : # Keep original absolute path if resolve fails but it's a target
                 unique_paths.append(p)
                 seen_paths_str.add(str(p))
            elif not p.is_absolute() and str(p) not in seen_paths_str: # Keep original relative path
                 unique_paths.append(p) # Store as is, will be checked with .exists() later
                 seen_paths_str.add(str(p))
...
def _resolve_embedding_path(embedding_path=None):
    ...
    search_paths = [
        ...
        Path("/home/ubuntu/mcs/templ_pipeline/data/embeddings/protein_embeddings_base.npz")
    ]
    ...
    return env_path or str(embedding_path) or "data/embeddings/protein_embeddings_base.npz"
...
def get_protein_sequence(pdb_file: str, target_chain_id: Optional[str] = None) -> Tuple[Optional[str], List[str]]:
    ...
    try:
        ...
        if seqres_sequences:
            ...
            if common_chains:
                ...
                if len(seqres_seq) == len(struct_seq):
                    ...
                    logger.debug(f"Using SEQRES sequence for chain {target_chain} (length: {len(final_seq)})")
                else:
                    logger.warning(
        ...
        if final_seq is None:
            ...
            used_chains = [target_chain]
            logger.debug(f"Using structure-based sequence for chain {target_chain} (length: {len(final_seq)})")
    ...
    if _esm_components is None:
        try:
            ...
            if gpu_info["available"]:
                logger.info(f"GPU detected: {gpu_info['device_name']} ({gpu_info['memory_total']:.1f}GB)")
...
def calculate_embedding(sequence: str) -> Optional[np.ndarray]:
    ...
    try:
        ...
        logger.info(f"Embedding generated successfully in {elapsed:.2f}s, shape: {embedding_array.shape}")
        ...
        if 'emb_cpu' in locals(): del emb_cpu
...
def get_protein_embedding(pdb_file: str, target_chain_id: Optional[str] = None) -> Tuple[Optional[np.ndarray], List[str]]:
    ...
    try:
        ...
        if not target_chain_id and chains:
            ...
            logger.info(f"No chain specified, using first available chain: {target_chain_id}")
        
        logger.info(f"Extracted sequence of length {len(seq)} from chain(s): {', '.join(chains)}")
        ...
        elapsed = time.time() - start_time
        if emb is not None:
            logger.info(f"Successfully generated embedding for {pdb_id} in {elapsed:.2f}s")
        else:
            logger.error(f"Failed to generate embedding for {pdb_id} after {elapsed:.2f}s")
            
        return emb, chains
        
    except Exception as e:
        elapsed = time.time() - start_time
        logger.error(f"Failed to calculate embedding for {pdb_file} after {elapsed:.2f}s: {str(e)}")
        import traceback
        logger.error(traceback.format_exc())
        return None, []
...
class EmbeddingManager:
    ...
    def __init__(self, 
        ...
        if cache_dir:
            ...
            self.cache_dir = str(Path("~/.cache/templ/embeddings").expanduser().resolve())
        ...
        if self.use_cache:
            ...
            self.cache_dir = str(model_specific_cache_dir) # Update to model-specific path
    ...
    def _load_embeddings(self) -> bool:
        ...
        if not self.embedding_path or not os.path.exists(self.embedding_path):
            logger.warning(f"Embedding file not found or path is invalid: '{self.embedding_path}'")
            return False
        ...
        try:
            ...
            for i, pid in enumerate(pdb_ids):
                ...
                if chain_ids is not None:
                    ...
                    self.embedding_chain_data[pid_upper] = "" # Default if no chain data
            
            logger.info(f"Loaded {len(self.embedding_db)} embeddings from {self.embedding_path}")
            return True
        except Exception as e:
            logger.error(f"Error loading embeddings from '{self.embedding_path}': {str(e)}")
            return False
    ...
    def _get_cache_path(self, pdb_id: str, chain_id: Optional[str] = None) -> str:
        ...
        if not self.cache_dir: # Should not happen if use_cache is True
            ...
            return os.path.join(tempfile.gettempdir(), f"{pdb_id}_{chain_id or 'all'}.npz")

        # Sanitize pdb_id and chain_id to be safe filenames
        safe_pdb_id = "".join(c if c.isalnum() or c in ('_', '-') else '_' for c in pdb_id)
        safe_chain_id = "".join(c if c.isalnum() or c in ('_', '-') else '_' for c in chain_id) if chain_id else "allchains"
    ...
    def _save_to_cache(self, pdb_id: str, embedding: np.ndarray, chains: List[str]) -> bool:
        ...
        try:
            ...
            cache_path = self._get_cache_path(pdb_id, cache_key_chain) # Use consistent chain for cache key
    ...
    def _load_from_cache(self, pdb_id: str, target_chain_id: Optional[str] = None) -> Tuple[Optional[np.ndarray], Optional[str]]:
        ...
        try:
            ...
            if os.path.exists(cache_path_specific_chain):
                ...
                logger.debug(f"Loaded embedding from cache (specific chain key {target_chain_id}): {cache_path_specific_chain}")
                return data['embedding'], str(data['chain_ids']) if 'chain_ids' in data else ""

            # Try with a general key if specific chain not found or not provided
            cache_path_general = self._get_cache_path(pdb_id) # Default chain key (e.g., "allchains")
            if os.path.exists(cache_path_general):
                data = np.load(cache_path_general, allow_pickle=True)
                logger.debug(f"Loaded embedding from general cache key: {cache_path_general}")
                return data['embedding'], str(data['chain_ids']) if 'chain_ids' in data else ""
    ...
    def clear_cache(self) -> bool:
        ...
        try:
            if not self.use_cache or not self.cache_dir or not os.path.exists(self.cache_dir) :
                logger.info("Cache not used or cache directory does not exist. Nothing to clear.")
                return False
    ...
    def get_cache_stats(self) -> Dict[str, Any]:
        ...
        try:
            if not self.use_cache or not self.cache_dir or not os.path.exists(self.cache_dir):
                return {"enabled": self.use_cache, "count": 0, "size_mb": 0, "path": self.cache_dir or "Not configured"}
            ...
            return {"enabled": self.use_cache, "error": str(e), "path": self.cache_dir or "Error in path"}
    ...
    def has_embedding(self, pdb_id: str) -> bool:
        ...
        """
        # Normalize PDB ID
        pdb_id = pdb_id.upper().split(':')[-1]  # Normalize to uppercase for consistent lookup
        ...
        if not self.embedding_db and self.embedding_path and os.path.exists(self.embedding_path):
            logger.warning(f"Embedding DB seems empty. Trying to reload from {self.embedding_path}")
            self._load_embeddings()
    ...
    def get_embedding(self, pdb_id: str, pdb_file: Optional[str] = None, 
        ...
        # 3. Check cache
        if self.use_cache:
            cached_emb, cached_chains_str = self._load_from_cache(pdb_id, target_chain_id)
        ...
        if not actual_pdb_file_to_use and not pdb_id.endswith(".pdb") and not os.path.exists(pdb_id):
            ...
            logger.warning(f"No PDB file provided for {pdb_id} to generate on-demand embedding, and not found in DB/cache.")
            return None, None
        elif pdb_id.endswith(".pdb") and os.path.exists(pdb_id):  # If pdb_id is actually a path
        ...
        if actual_pdb_file_to_use and os.path.exists(actual_pdb_file_to_use):
            ...
            logger.info(f"Generating on-demand embedding for {pdb_id} from file {actual_pdb_file_to_use}")
            
            seq, chains_list = get_protein_sequence(actual_pdb_file_to_use, target_chain_id)
            if not seq or not chains_list:
                logger.error(f"Failed to extract sequence from {actual_pdb_file_to_use} for {pdb_id}")
                return None, None
            ...
            if embedding is not None:
                ...
                if self.use_cache:
                    self._save_to_cache(pdb_id, embedding, chains_list)  # Save with list of chains
                    
                logger.info(f"Successfully generated and cached on-demand embedding for {pdb_id}")
    ...
    def find_neighbors(self, 
                      ...
                      allow_self_as_template: bool = False) -> List[Union[str, Tuple[str, float]]]:
        """
        ...
        if query_embedding is None:
            query_embedding, _ = self.get_embedding(query_pdb_id, pdb_file=query_pdb_file, target_chain_id=query_target_chain_id)
            if query_embedding is None:
                logger.error(f"No embedding found or generated for query PDB {query_pdb_id}. Cannot find neighbors.")
                return []
        ...
        if not self.embedding_db:
             logger.warning("Embedding database (self.embedding_db) is empty. Cannot find neighbors from it.")
        ...
        for pid, emb in self.embedding_db.items():
            ...
            if allowed_pdb_ids is not None and pid_norm not in allowed_pdb_ids:
                continue
            if exclude_uniprot_ids and pid_norm in self.pdb_to_uniprot and self.pdb_to_uniprot[pid_norm] in exclude_uniprot_ids:
                continue
        ...
        if not all_embeddings_list:
            logger.warning("No valid template candidates found in self.embedding_db after filtering.")
            return []
        ...
        neighbor_candidates = sorted(list(zip(all_candidate_pdb_ids, similarities)), key=lambda x: x[1], reverse=True)
        
        if similarity_threshold is not None:
            final_neighbors = [(pid, sim) for pid, sim in neighbor_candidates if sim >= similarity_threshold]
        elif k is not None:
    ...
    def get_chain_data(self, pdb_id: str) -> Optional[str]:
        ...
        logger.debug(f"Chain data for {pdb_id} not found in direct memory (embedding_chain_data or on_demand_chain_data).")
        return None
    ...
    def process_batch(self) -> int:
        ...
        for item in self._batch_queue:
            try:
                sequence, chains = get_protein_sequence(item['pdb_file'], item['target_chain_id'])
                if sequence:
        ...
        try:
            ...
            for i, seq in enumerate(sequences):
                try:
                    ...
                    token_representations = results["representations"][33][i, 1:len(seq)+1]
...
def get_embedding(pdb_id_or_file: str, embedding_path: Optional[str] = None, 
                  chain_id: Optional[str] = None) -> Union[np.ndarray, Tuple[np.ndarray, str]]:
    """
    ...
    if not is_file:
        ...
        if not manager.has_embedding(pdb_id):
            raise ValueError(f"PDB ID {pdb_id} not found in pre-computed embedding database or cache")
    
    # Try to get embedding from database or generate
    embedding_result = manager.get_embedding(pdb_id, pdb_file=pdb_id_or_file if is_file else None, target_chain_id=chain_id)
...
def select_templates(
    ...
    query_pdb_file: Optional[str] = None, # To pass to find_neighbors if query_embedding needs generation
    ...
    """
    manager = EmbeddingManager(embedding_path=embedding_path) # Manager resolves its own path
    ...
    if enable_uniprot_exclusion:
        if pdb_to_uniprot_file and os.path.exists(pdb_to_uniprot_file):
            ...
            try:
                ...
                logger.error(f"Error decoding JSON from pdb_to_uniprot_file: {pdb_to_uniprot_file}")
            except Exception as e:
                logger.error(f"Error processing pdb_to_uniprot_file {pdb_to_uniprot_file}: {e}")

        if exclude_uniprot_ids_file and os.path.exists(exclude_uniprot_ids_file):
            with open(exclude_uniprot_ids_file) as f:
                final_exclude_uniprot_ids = {line.strip().upper() for line in f if line.strip()}
    
    # Handle PDB filtering (allowed list)
    final_allowed_pdb_ids: Optional[Set[str]] = None
    if enable_pdb_filtering and allowed_pdb_ids_file and os.path.exists(allowed_pdb_ids_file):
...
def analyze_embedding_database(embedding_path: Optional[str] = None) -> Dict[str, Any]:
    ...
    try:
        ...
        logger.info(f"Analyzing embedding file: {resolved_path} (Size: {file_size:.2f} MB, Modified: {mod_time})")
        ...
        with np.load(resolved_path, allow_pickle=True) as data:
            ...
            if has_pdb_ids:
                ...
                if pdb_count > 0:
                    ...
                    if pdb_count <= 10:
                        ...
                        samples = [0, 1, 2, pdb_count//2-1, pdb_count//2, pdb_count//2+1, pdb_count-3, pdb_count-2, pdb_count-1]
                        
                    pdb_sample = [str(pdb_ids[i]) for i in samples if i < pdb_count]
                    logger.info(f"Sample PDB IDs: {pdb_sample[:5]} ... (total: {len(pdb_sample)})")
                    ...
                    for pid in pdb_ids:
                        ...
                        pdb_first_chars[first_char] = pdb_first_chars.get(first_char, 0) + 1
            ...
            if has_chain_ids:
                ...
                chain_id_count = len(chain_ids) if isinstance(chain_ids, np.ndarray) else 0
                logger.info(f"Found {chain_id_count} chain_ids in database")
                
                # Check a sample
                if chain_id_count > 0:
                    chain_sample = [str(chain_ids[i]) for i in samples[:3] if i < chain_id_count]
        ...
        return {
            "status": "error",
            "message": str(e),
            "resolved_path": _resolve_embedding_path(embedding_path) if embedding_path else "Not provided"
        }
...
def get_templates_with_progressive_fallback(all_templates: List[Any], 
                                          fallback_thresholds: List[float] = None) -> Tuple[List[Any], float, bool]:
    ...
    for threshold in fallback_thresholds:
        ...
        if filtered_templates:
            if threshold > CA_RMSD_THRESHOLD:
                logger.warning(f"Using relaxed CA RMSD threshold ({threshold}Å) - found {len(filtered_templates)} templates (poses may be less accurate)")
            else:
                logger.info(f"Found {len(filtered_templates)} templates with CA RMSD ≤ {threshold}Å")
    ...
    if best_template:
        logger.warning(f"Using central atom fallback with best available template (CA RMSD: {best_rmsd}Å)")
