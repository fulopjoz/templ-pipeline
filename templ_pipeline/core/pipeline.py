...
class TEMPLPipeline:
    ...
    def __init__(
        ...
        if config is not None:
            ...
            effective_output_dir = (
                config.output_dir if config.output_dir else output_dir
            )
        else:
        ...
        self.output_manager = EnhancedOutputManager(
            base_output_dir=self.output_dir, run_id=self.run_id
        )
    ...
    def _extract_pdb_id_from_path(self, file_path: str) -> Optional[str]:
        ...
        try:
                    ...
                    if line.startswith("HEADER"):
                        ...
                        if len(header_parts) >= 2:
                            ...
                            if len(potential_id) == 4 and potential_id.isalnum():
                                ...
                                log.info(
                                    f"Extracted PDB ID '{pdb_id}' from simple header format"
                                )
                                return pdb_id
            ...
            log.warning(
                f"No valid PDB ID found in file header or filename: {file_path}"
            )
            return None
        except Exception as e:
    ...
    def load_target_data(self) -> bool:
        ...
        try:
            ...
            if not pdb_to_load and target_pdb_id:
                ...
                pdb_to_load = pdb_path(
                    target_pdb_id, getattr(self.config, "data_dir", DEFAULT_DATA_DIR)
                )
            ...
            target_smiles = getattr(self.config, "target_smiles", None) or getattr(
                self.config, "ligand_smiles", None
            )
            ...
            if target_smiles:
                ...
                if self.target_mol is None:
                    log.error(
                        f"Failed to create target molecule from SMILES: {target_smiles}"
                    )
                    return False
                ...
                if not is_valid:
                    ...
                    try:
                        ...
                        log.warning(
                            f"Molecule validation failed for {pdb_id}: {validation_msg}"
                        )
                        ...
                        if not validation_msg.lower().startswith("filtered:"):
                            validation_msg = f"FILTERED: {validation_msg}"
                        log.error(
                            f"Target molecule validation failed: {validation_msg}"
                        )
                        return False
            elif ligand_file:
                ...
                if mols:
                    ...
                    log.info(
                        f"Loaded target molecule from SDF: {ligand_file} with {self.target_mol.GetNumAtoms()} atoms"
                    )
                else:
                    log.error(
                        f"Failed to load any valid molecule from SDF: {ligand_file}"
                    )
                    return False
            else:
    ...
    def _load_crystal_molecule(self, pdb_id: str) -> Optional[Chem.Mol]:
        ...
        try:
            ...
            ligands_sdf_gz = (
                f"{actual_data_dir}/ligands/templ_processed_ligands_v1.0.0.sdf.gz"
            )
            ...
            if not os.path.exists(ligands_sdf_gz):
                log.warning(
                    f"CRYSTAL_LOADING: Crystal ligand SDF not found: {ligands_sdf_gz}"
                )
                return None
            ...
            with gzip.open(ligands_sdf_gz, "rb") as fh:
                for mol in Chem.ForwardSDMolSupplier(
                    fh, removeHs=False, sanitize=False
                ):
                    ...
                    if mol_name and mol_name.lower().startswith(pdb_id.lower()):
                        ...
                        log.info(
                            f"CRYSTAL_LOADING:   Molecule atoms: {mol.GetNumAtoms()}"
                        )
                        log.info(
                            f"CRYSTAL_LOADING:   Molecule conformers: {mol.GetNumConformers()}"
                        )
                        log.info(
                            f"CRYSTAL_LOADING:   Searched {molecules_searched} molecules total"
                        )
                        return Chem.Mol(mol)
            ...
            log.error(
                f"CRYSTAL_LOADING: Failed to load crystal molecule for {pdb_id}: {e}"
            )
            import traceback
    ...
    def load_templates(self) -> bool:
        ...
        try:
            ...
            log.info(
                f"Template filtering: {original_count} → {len(self.templates)} templates"
            )
            log.info(
                f"Filtered out: {filtered_peptides} peptides, {filtered_polysaccharides} polysaccharides ({total_filtered} total)"
            )
    ...
    def get_protein_embedding(
        self, pdb_id: str, pdb_file: Optional[str] = None
        ...
        try:
            ...
            if pdb_id and ("/" in pdb_id or "\\" in pdb_id or pdb_id.endswith(".pdb")):
                log.warning(
                    f"File path detected as PDB ID: '{pdb_id}'. Attempting to extract PDB ID from path."
                )
                ...
                if extracted_id:
                    ...
                    pdb_id = extracted_id
                    log.info(
                        f"Extracted PDB ID '{pdb_id}' from path '{original_input}'"
                    )
                else:
            ...
            if pdb_file and os.path.exists(pdb_file):
                ...
                if extracted_pdb_id:
                    ...
                    log.info(
                        f"Using PDB ID '{pdb_id}' extracted from file for embedding lookup"
                    )
            ...
            if pdb_file and os.path.exists(pdb_file):
                log.info(
                    f"No existing embedding for '{pdb_id}'. Generating new embedding from {pdb_file}."
                )
                embedding, chains_str = self.embedding_manager.get_embedding(
                    pdb_id, pdb_file=pdb_file
                )
                if embedding is not None:
                    return embedding, (chains_str.split(",") if chains_str else [])
            ...
            log.error(
                f"Failed to get protein embedding for {pdb_id}: {e}", exc_info=True
            )
            return None, None
    ...
    def find_similar_templates(
        ...
        k: int = 100,
        exclude_pdb_ids: Optional[set] = None,
        ...
        try:
            ...
            self.similarity_search_info = {
                ...
                "embedding_dimension": (
                    query_embedding.shape[0] if query_embedding is not None else None
                ),
                "exclude_pdb_ids_count": len(exclude_pdb_ids) if exclude_pdb_ids else 0,
                "allowed_pdb_ids_count": (
                    len(allowed_pdb_ids) if allowed_pdb_ids else None
                ),
            }
    ...
    def process_templates(
            ...
            try:
                ...
                if template_mol is None:
                    if i < 3:
                        ...
                        log.warning(
                            f"Template molecule not found for {pdb_id}. Sample properties: {props}"
                        )
                    else:
                ...
                if self.reference_protein is not None:
                    ...
                    if (
                        embedding_similarities
                        and pdb_id.upper() in embedding_similarities
                    ):
                    ...
                    if transformed_mol is not None:
                        ...
                        if transformed_mol.HasProp("alignment_method"):
                            alignment_method = transformed_mol.GetProp(
                                "alignment_method"
                            )
        ...
        log.info(
            f"Successfully processed {len(processed_templates)} templates from {len(template_pdb_ids)} candidates"
        )
        if failed_templates:
            log.info(
                f"Failed to process {len(failed_templates)} templates: {failed_templates[:5]}{'...' if len(failed_templates) > 5 else ''}"
            )
    ...
    def filter_templates_by_rmsd(self, templates: List[Chem.Mol]) -> List[Chem.Mol]:
        ...
        try:
            ...
            ca_rmsd_threshold = getattr(
                self.config, "ca_rmsd_threshold", DEFAULT_CA_RMSD_THRESHOLD
            )

            # Apply progressive fallback filtering
            filtered_templates, threshold_used, use_central_atom = (
                get_templates_with_progressive_fallback(
            ...
            if filtered_templates:
                log.info(
                    f"Filtered to {len(filtered_templates)} templates using CA RMSD threshold {threshold_used}Å"
                )
                return filtered_templates
            else:
                log.warning(
                    "No templates passed RMSD filtering, returning original templates"
                )
                return templates
    ...
    def generate_conformers(
        self, template_mol: Chem.Mol, mcs_smarts: str, num_conformers: int
    ) -> Optional[Chem.Mol]:
        ...
        try:
            ...
            log.info(
                f"Pipeline generate_conformers: enable_optimization={enable_optimization}, unconstrained={unconstrained}"
            )
            ...
            if unconstrained or mcs_smarts == "*":
                ...
                conformers = central_atom_embed(
                    self.target_mol, template_mol, num_conformers, enable_optimization
                )
            else:
    ...
    def score_conformers(self, conformers: Chem.Mol, template_mol: Chem.Mol) -> Union[
                ...
                for metric, (pose, scores) in results.items():
                    if pose is not None:
                        enhanced_pose = generate_properties_for_sdf(
                            pose, metric, scores[metric], output_name
                        )
    ...
    def run_full_pipeline(
        ...
        output_folder = (
            str(self.output_manager.timestamped_folder)
            if self.output_manager.timestamped_folder
        ...
        return {
            ...
            "filtering_info": {
                ...
                "allowed_pdb_ids": (
                    list(getattr(self, "allowed_pdb_ids_used", set()))
                    if getattr(self, "allowed_pdb_ids_used", None) is not None
                    else None
                ),
        ...
        try:
            ...
            if not query_pdb_id and target_pdb_file:
                ...
                if extracted_id:
                    ...
                    log.info(
                        f"Using extracted PDB ID '{query_pdb_id}' for pipeline execution"
                    )
                else:
                    # Use the filename without extension as fallback
                    query_pdb_id = os.path.splitext(os.path.basename(target_pdb_file))[
                        0
                    ]
            ...
            query_embedding, ref_chains = self.get_protein_embedding(
                query_pdb_id, pdb_file=target_pdb_file
            )

            if query_embedding is None:
                log.error(
                    f"Failed to get/generate protein embedding for {query_pdb_id}"
                )
                return False
            ...
            if not ref_chains:
                log.warning(
                    f"No chains returned with embedding for {query_pdb_id}. Attempting to extract from file."
                )
                if target_pdb_file and os.path.exists(target_pdb_file):
                    ...
                    if self.embedding_manager is not None:
                        ...
                        if chains_str:
                            ...
                            log.info(
                                f"Successfully retrieved chains from embedding metadata: {ref_chains}"
                            )
                        else:
                            log.error(
                                f"No PDB file available and no chain data in embedding for {query_pdb_id}"
                            )
                            return False
                    else:
                        log.error(
                            f"Embedding manager is None and no PDB file available for {query_pdb_id}"
                        )
                        return False
            ...
            similar_template_ids, embedding_similarities = self.find_similar_templates(
                query_pdb_id,
                query_embedding,
                k=num_templates,
                exclude_pdb_ids=exclude_pdb_ids,
            ...
            log.info(
                f"Found {len(similar_template_ids)} similar templates for {query_pdb_id}"
            )
            ...
            self.allowed_pdb_ids_used = (
                allowed_pdb_ids.copy() if allowed_pdb_ids is not None else None
            )
            ...
            if allowed_pdb_ids is not None:
                ...
                if query_pdb_id.upper() in [
                    pid.upper() for pid in similar_template_ids
                ]:
                    log.warning(
                        f"Timesplit constraint validation: target {query_pdb_id} found in template list - this may indicate constraint bypass"
                    )
                else:
                    log.info(
                        f"Timesplit constraint validation: target {query_pdb_id} properly excluded from template list"
                    )

                # Validate all templates are within allowed set
                invalid_templates = [
                    pid
                    for pid in similar_template_ids
                    if pid.upper() not in allowed_pdb_ids
                ]
                if invalid_templates:
                    log.error(
                        f"Constraint violation: templates {invalid_templates} not in allowed set"
                    )
                else:
                    log.info(
                        f"Constraint validation passed: all {len(similar_template_ids)} templates within allowed set"
                    )
            ...
            if (
                allowed_pdb_ids is None
                and query_pdb_id.upper()
                not in [pid.upper() for pid in similar_template_ids]
                and (
                    exclude_pdb_ids is None
                    or query_pdb_id.upper() not in exclude_pdb_ids
                )
            ):
                ...
                if target_embedding is not None:
                    ...
                    log.info(
                        f"Added target {query_pdb_id} to template list as it exists in database"
                    )
            elif allowed_pdb_ids is not None:
                # In constrained scenarios (like timesplit), respect the constraints strictly
                log.info(
                    f"Template constraints active: target {query_pdb_id} excluded to maintain benchmark integrity"
                )
            ...
            if query_pdb_id.upper() in [pid.upper() for pid in similar_template_ids]:
                ...
                if allowed_pdb_ids is not None:
                    log.info(
                        f"Target {query_pdb_id} found as legitimate template within constraints, using native pose."
                    )
                else:
                    log.info(
                        f"Target {query_pdb_id} is in templates, using its native pose."
                    )
                native_ligand_path = ligand_path(query_pdb_id, "data")
                if native_ligand_path and os.path.exists(native_ligand_path):
                    try:
                        native_ligand = Chem.SDMolSupplier(
                            native_ligand_path, removeHs=False
                        )[0]
                        if native_ligand:
                            native_ligand.SetProp("template_pid", query_pdb_id)
                            native_ligand.SetProp(
                                "ca_rmsd", "0.0"
                            )  # Native pose has 0 RMSD
                            native_ligand.SetProp(
                                "similarity_score", "1.0"
                            )  # Native pose has 1.0 similarity
                            native_ligand.SetProp(
                                "alignment_method", "native"
                            )  # Native template uses no alignment
                            native_ligand.SetProp(
                                "anchor_count", "N/A"
                            )  # Native template has no alignment anchors
                            ...
                            similar_template_ids = [
                                pid
                                for pid in similar_template_ids
                                if pid.upper() != query_pdb_id.upper()
                            ]
                        else:
                            log.warning(
                                f"Could not load native ligand for {query_pdb_id} from {native_ligand_path}"
                            )
                    except Exception as e:
                        log.warning(f"Error loading native ligand: {e}")
                else:
                    log.warning(
                        f"Native ligand file not found for {query_pdb_id} at {native_ligand_path}"
                    )

            # Process remaining templates
            if similar_template_ids:
                transformed_ligands_from_templates = self.process_templates(
                    similar_template_ids, ref_chains, embedding_similarities
                )
                ...
                self.template_processing_stats = {
                    ...
                    "processing_failed": len(similar_template_ids)
                    - len(transformed_ligands_from_templates),
                }

                # Apply RMSD filtering with progressive fallback
                filtered_templates = self.filter_templates_by_rmsd(
                    transformed_ligands_from_templates
                )
                ...
                self.template_filtering_stats = {
                    ...
                    "removed_by_rmsd_filter": len(transformed_ligands_from_templates)
            ...
            if hasattr(self, "protein_alignment_info"):
                ...
                self.protein_alignment_info["alignment_methods_breakdown"][
                    "native"
                ] = native_poses_used
            else:
                ...
                self.protein_alignment_info = {
                    ...
                    "alignment_methods_breakdown": {
                        "homologs": 0,
                        "sequence": 0,
                        "3di": 0,
                        "centroid": 0,
                        "native": native_poses_used,
                    },
                }
            ...
            pre_rmsd_filtering_count = (
                len(transformed_ligands_from_templates)
                if "transformed_ligands_from_templates" in locals()
                else 0
            )
            post_rmsd_filtering_count = (
                len(filtered_templates) if "filtered_templates" in locals() else 0
            )

            self.mcs_analysis_input = {
                "total_templates_for_mcs": len(all_transformed_ligands),
                "native_templates": native_poses_used,
                "superimposed_templates": len(all_transformed_ligands)
                - native_poses_used,
            ...
            find_mcs_result = find_mcs(
                self.target_mol, all_transformed_ligands, return_details=True
            )
            ...
            log.info(
                f"Best template found: {best_template.GetProp('template_pid')} with MCS: {mcs_smarts}"
            )

            num_conformers = getattr(self.config, "n_confs", 200)
            target_with_conformers = self.generate_conformers(
                best_template, mcs_smarts, num_conformers
            )
            if (
                not target_with_conformers
                or target_with_conformers.GetNumConformers() == 0
            ):
                log.error("Failed to generate conformers.")
                return False
            log.info(
                f"Generated {target_with_conformers.GetNumConformers()} conformers."
            )
            ...
            for metric in metrics:
                ...
                if isinstance(all_ranked_poses, list):
                    ...
                    for metric_name, (mol, scores) in all_ranked_poses.items():
                        if (
                            metric_name == metric
                            and metric in scores
                            and scores[metric] > best_score
                        ):
                            best_score = scores[metric]
                            best_pose = (mol, scores)
            ...
            if hasattr(self, "pipeline_template_info"):
                ...
                embedding_similarity = (
                    best_template.GetProp("similarity_score")
                    if best_template.HasProp("similarity_score")
                    else "unknown"
                )
                alignment_method = (
                    best_template.GetProp("alignment_method")
                    if best_template.HasProp("alignment_method")
                    else "unknown"
                )
                anchor_count = (
                    best_template.GetProp("anchor_count")
                    if best_template.HasProp("anchor_count")
                    else "unknown"
                )

                self.pipeline_template_info = {
                    "best_template_pdb": (
                        best_template.GetProp("template_pid")
                        if best_template.HasProp("template_pid")
                        else "unknown"
                    ),
                    "num_conformers_generated": target_with_conformers.GetNumConformers(),
                    "ca_rmsd": (
                        best_template.GetProp("ca_rmsd")
                        if best_template.HasProp("ca_rmsd")
                        else "unknown"
                    ),
                    ...
                    "atoms_matched": (
                        mcs_details.get("atom_count", 0)
                        if "mcs_details" in locals() and isinstance(mcs_details, dict)
                        else 0
                    ),
                }
            ...
            if mcs_details:
                ...
                mcs_details_organized["mcs_similarity"] = mcs_details.get(
                ...
                total_query_atoms = (
                    len(self.target_mol.GetAtoms()) if self.target_mol else 0
                )
                total_template_atoms = (
                    len(best_template.GetAtoms()) if best_template else 0
                )
                ...
                mcs_details_organized["mcs_coverage_query"] = (
                    round(
                        mcs_details_organized["atom_count"] / total_query_atoms * 100, 1
                    )
                    if total_query_atoms > 0
                    else 0.0
                )
                mcs_details_organized["mcs_coverage_template"] = (
                    round(
                        mcs_details_organized["atom_count"]
                        / total_template_atoms
                        * 100,
                        1,
                    )
                    if total_template_atoms > 0
                    else 0.0
                )

                # Add atom mappings at the end
                mcs_details_organized["query_atoms"] = mcs_details.get(
                    "query_atoms", []
                )
                mcs_details_organized["template_atoms"] = mcs_details.get(
                    "template_atoms", []
                )
...
def run_pipeline(config: PipelineConfig) -> bool:
    ...
    pipeline = TEMPLPipeline(
        embedding_path=config.embedding_npz, output_dir=config.output_dir
    )
...
def run_pipeline_from_args(args) -> bool:
    ...
    try:
        config = PipelineConfig(
            ...
            ca_rmsd_threshold=getattr(
                args, "ca_rmsd_threshold", DEFAULT_CA_RMSD_THRESHOLD
            ),
        )
...
def run_from_pdb_and_smiles(
    pdb_id: str, smiles: str, output_dir: str = DEFAULT_OUTPUT_DIR
) -> bool:
    """Convenience function to run pipeline from PDB ID and SMILES."""
    try:
        config = PipelineConfig(
            protein_pdb_id=pdb_id, target_smiles=smiles, output_dir=output_dir
        )
