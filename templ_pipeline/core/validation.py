...
class SplitDataValidator:
    ...
    def validate_split_completeness(self) -> Dict[str, Any]:
        ...
        validation_result = {
            ...
            "has_overlaps": len(train_val_overlap) > 0
        ...
        if any(
            count == 0 for count in [len(train_pdbs), len(val_pdbs), len(test_pdbs)]
        ):
            validation_result["is_valid"] = False
...
class DatabaseValidator:
    ...
    def __init__(self, data_dir: str = "data"):
        ...
        self.ligands_path = (
            self.data_dir / "ligands" / "templ_processed_ligands_v1.0.0.sdf.gz"
        )
    ...
    def validate_file_integrity(self) -> Dict[str, Any]:
        ...
        if not all(
            result["valid"] for result in results.values() if isinstance(result, dict)
        ):
    ...
    def _validate_embeddings_file(self) -> Dict[str, Any]:
        ...
        try:
            ...
            return {
                ...
                "pdb_count": pdb_count,
                "embedding_dimension": (
                    data["embeddings"][0].shape[0] if pdb_count > 0 else 0
                ),
        ...
        try:
            ...
            try:
                ...
                return {
                    ...
                    "consistency_ratio": len(common_ids)
                    / max(len(embedding_pdb_ids), len(ligand_pdb_ids)),
                }
        ...
        try:
            ...
            if mol is None:
                result["errors"].append(
                    f"Failed to create molecule from SMILES: {test_case['smiles']}"
                )
                return result
...
def validate_pipeline_components(
    embedding_path: str, ligands_path: str, splits_dir: Optional[str] = None
) -> Dict[str, Any]:
    """
    ...
    try:
        ...
        results["molecular_validation"] = mol_validator.run_validation_suite(
            lambda x: x
        )
