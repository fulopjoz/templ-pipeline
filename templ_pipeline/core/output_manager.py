...
class EnhancedOutputManager:
    ...
    def get_output_path(self, filename: str) -> Path:
        ...
        if self.timestamped_folder is None:
            raise ValueError(
                "Output folder not set up. Call setup_output_folder() first."
            )
    ...
    def _create_enhanced_pose(
        ...
        original_conformer_id: Optional[int] = None,
    ) -> Chem.Mol:
        """
    ...
    def _calculate_rmsd(self, pose: Chem.Mol, crystal: Chem.Mol) -> float:
        ...
        try:
            ...
            if pose_clean.GetNumAtoms() != crystal_clean.GetNumAtoms():
                log.debug(
                    f"RMSD skipped: atom count mismatch "
                    f"(pose: {pose_clean.GetNumAtoms()}, "
                    f"crystal: {crystal_clean.GetNumAtoms()})"
                )
                return float("nan")
            ...
            if pose_formula != crystal_formula:
                log.debug(
                    f"RMSD skipped: molecular formula mismatch "
                    f"(pose: {pose_formula}, crystal: {crystal_formula})"
                )
                return float("nan")
    ...
    def _validate_poses_structure(self, poses: Any) -> bool:
        ...
        if not actual_metrics.intersection(expected_metrics):
            log.warning(
                f"No valid metric keys found. Expected one of {expected_metrics}, "
                f"got: {actual_metrics}"
            )
            return False
    ...
    def _validate_single_pose(self, metric: str, pose_data: Any) -> bool:
        ...
        if not isinstance(pose_data, (tuple, list)) or len(pose_data) != 2:
            log.warning(
                f"Pose data for {metric} should be tuple/list of length 2, "
                f"got: {type(pose_data)} with length "
                f"{len(pose_data) if hasattr(pose_data, '__len__') else 'N/A'}"
            )
            return False
        ...
        if metric not in scores:
            log.warning(
                f"Score for {metric} not found in scores dictionary: "
                f"{list(scores.keys())}"
            )
            return False
