...
from biotite.structure import AtomArray, filter_amino_acids, get_chains, superimpose, superimpose_homologs
import uuid
import random
    ...
    if logger.isEnabledFor(logging.DEBUG):
        ...
        for i, ref in enumerate(refs[:3]):  # Log first 3 for brevity
            ...
            if tgt_smiles == ref_smiles:
                logger.debug(f"MCS Debug: IDENTICAL SMILES FOUND with {ref_name}! This should get perfect MCS.")
    
    while True:  # Continue until we find something acceptable
        hits = []
        for i, r in enumerate(refs):
            # Handle organometallic atoms before MCS search
            tgt_processed, tgt_had_metals, tgt_subs = detect_and_substitute_organometallic(tgt, "target")
            ref_processed, ref_had_metals, ref_subs = detect_and_substitute_organometallic(r, f"template_{i}")
            
            # Debug organometallic processing
            if (tgt_had_metals or ref_had_metals) and logger.isEnabledFor(logging.DEBUG):
                logger.debug(f"MCS Debug: Organometallic processing - Target: {tgt_had_metals}, Ref {i}: {ref_had_metals}")
            ...
            try:
                ...
                if mcr:
                    ...
                    # Debug MCS result
                    if logger.isEnabledFor(logging.DEBUG):
                        logger.debug(f"MCS Debug: Ref {i} -> {len(atom_matches)} atoms matched, SMARTS: {smarts}")
                    ...
                    if logger.isEnabledFor(logging.DEBUG):
                        logger.debug(f"MCS Debug: No MCS found with ref {i} at threshold {opts.similarityThreshold}")
            except Exception as e:
                logger.warning(f"MCS search failed for molecule {i} at threshold {opts.similarityThreshold}: {str(e)}")
                continue
        
        if hits:
            # Enhanced template selection with molecular similarity tiebreaker
            best_idx, best_smarts, best_details = _select_optimal_template(tgt, refs, hits, return_details)
            
            if logger.isEnabledFor(logging.DEBUG):
                logger.debug(f"MCS Debug: Best match found - Size: {hits[0][0] if hits else 0}, Index: {best_idx}, Threshold: {opts.similarityThreshold}")
            ...
            if best_size >= min_acceptable_size or opts.similarityThreshold <= desperate_threshold:
                if logger.isEnabledFor(logging.DEBUG):
                    logger.debug(f"MCS found: size={best_size}, threshold={opts.similarityThreshold:.2f}")
                if return_details:
                    return best_idx, best_smarts, best_details
                return best_idx, best_smarts
            else:
                if logger.isEnabledFor(logging.DEBUG):
                    logger.debug(f"Rejecting small MCS (size={best_size}) at threshold {opts.similarityThreshold:.2f}")
        
        # Reduce threshold and continue
        if opts.similarityThreshold > 0.0:
            if logger.isEnabledFor(logging.DEBUG):
                logger.debug(f"No MCS at threshold {opts.similarityThreshold:.2f}, reducing…")
            opts.similarityThreshold -= 0.1
        else:
            # Central atom fallback: use template with best CA RMSD
            if logger.isEnabledFor(logging.DEBUG):
                logger.debug("MCS search failed - using central atom fallback with best CA RMSD template")
...
def _select_optimal_template(tgt: Chem.Mol, refs: List[Chem.Mol], hits: List, return_details: bool) -> Tuple:
    ...
    logger.info(f"MCS tiebreaker: selected template {best_candidate[1]} with similarity {best_similarity:.3f}")
...
def find_best_ca_rmsd_template(refs: List[Chem.Mol]) -> int:
    ...
    logger.info(f"Selected template {best_idx} with CA RMSD {best_rmsd:.3f}Å for central atom fallback")
    return best_idx

def prepare_mol(mol: Chem.Mol, remove_stereo: bool = False, simplify: bool = False) -> Optional[Chem.Mol]:
    ...
    try:
        ...
        if simplify:
            ...
            if hasattr(mol_copy, 'GetStereoGroups') and callable(getattr(mol_copy, 'GetStereoGroups')):
...
def generate_conformers(query_mol: Chem.Mol, template_mols: List[Chem.Mol], 
                      n_conformers: int = 100, n_workers: int = 1) -> Tuple[Optional[Chem.Mol], Optional[List[int]]]:
    ...
    query_name = query_mol.GetProp("_Name") if query_mol.HasProp("_Name") else "unnamed_query"
    template_names = [m.GetProp("_Name") if m.HasProp("_Name") else f"template_{i}" 
                     for i, m in enumerate(template_mols)]
    logger.info(f"Generating conformers for {query_name} using {len(template_mols)} templates: {', '.join(template_names[:3])}...")
    logger.info(f"Query molecule: {query_mol.GetNumAtoms()} atoms, {query_mol.GetNumBonds()} bonds")
    
    # Find the best template and MCS
    try:
        best_template_idx, mcs_smarts, mcs_details = find_mcs(query_mol, template_mols, return_details=True)
        
        if best_template_idx is None or mcs_smarts is None:
            logger.warning(f"Could not find MCS between query {query_name} and templates {', '.join(template_names[:3])}")
            # Add more detailed diagnostics about molecular properties
            for i, template in enumerate(template_mols[:3]):  # Just check first 3 for brevity
                template_name = safe_name(template, f"template_{i}")
                logger.info(f"Template {template_name}: {template.GetNumAtoms()} atoms, {template.GetNumBonds()} bonds")
                ...
                try:
                    ...
                    logger.info(f"Descriptor comparison - Query: {query_rings} rings, {query_frags} fragments | "
        ...
        best_template_name = best_template.GetProp("_Name") if best_template.HasProp("_Name") else f"template_{best_template_idx}"
        logger.info(f"Selected template {best_template_name} with MCS SMARTS: {mcs_smarts}")
        ...
        try:
            ...
            if conformers_mol is not None and conformers_mol.GetNumConformers() > 0:
                logger.info(f"Successfully generated {conformers_mol.GetNumConformers()} conformers")
                ...
                logger.warning(f"Constrained embedding failed to produce conformers for template {best_template_name}")
                    ...
                    if mcs_smarts != "*":  # Skip analysis for central atom fallback
                        ...
                        if mcs_patt:
                            ...
                            if query_match and template_match:
                                ...
                                template_coverage = match_size / best_template.GetNumAtoms()
                                
                                logger.info(f"MCS match analysis: {match_size} atoms matched "
                                          f"({query_coverage:.1%} of query, {template_coverage:.1%} of template)")
                                
                                if query_coverage < 0.3 or template_coverage < 0.3:
                                    logger.warning("MCS coverage is too low for reliable conformer generation")
                except Exception as diagnostic_err:
                    logger.debug(f"Error in template diagnostic analysis: {str(diagnostic_err)}")
                
                # DEBUG: Log mcs_atom_indices structure even on failure
                logger.info(f"DEBUG: mcs_atom_indices type (failed case): {type(mcs_atom_indices)}")
                logger.info(f"DEBUG: mcs_atom_indices content (failed case): {mcs_atom_indices}")
                return None, mcs_atom_indices
        except Exception as e:
            logger.error(f"Error during conformer generation with template {best_template_name}: {str(e)}")
            return None, mcs_atom_indices
    except Exception as e:
...
def _mmff_minimize_single_conformer_task(args_tuple):
    ...
    try:
        ...
        if mol is None:
            logger.debug(f"_mmff_task: Could not create mol from MolBlock for conf {conformer_id}")
        ...
        try:
            ...
            logger.debug(f"_mmff_task: Sanitization warning for conf {conformer_id}: {san_e}")
        ...
        if mol.GetNumConformers() == 0:
            logger.debug(f"_mmff_task: No conformers found in MolBlock for conf {conformer_id}, embedding")
            AllChem.EmbedMolecule(mol, randomSeed=42)
        # Work on first conformer (ID 0)
        
        mp = rdForceFieldHelpers.MMFFGetMoleculeProperties(mol, mmffVariant=mmff_variant)
        if mp is None:
            logger.debug(f"_mmff_task: Could not get MMFF params for conf {conformer_id}")
            return conformer_id, None
            
        ff = rdForceFieldHelpers.MMFFGetMoleculeForceField(mol, mp, confId=0) # Use the single conformer (ID 0)
        if ff is None:
            logger.debug(f"_mmff_task: Could not get MMFF force field for conf {conformer_id}")
            return conformer_id, None
        ...
        for idx in fixed_atom_indices:
            if idx < mol.GetNumAtoms(): # Ensure fixed index is valid
                ...
                logger.warning(f"_mmff_task: Fixed atom index {idx} out of bounds for {mol.GetNumAtoms()} atoms in conf {conformer_id}")
        
        ff.Minimize(maxIts=max_its)
        minimized_coords = mol.GetConformer(0).GetPositions().tolist()
        return conformer_id, minimized_coords
    except Exception as e:
        logger.error(f"_mmff_task: MMFF Minimization uncaught exception for conf {conformer_id}: {e} {traceback.format_exc()}")
        return conformer_id, None

# Sequential MMFF minimization version - kept for reference and fallback
def mmff_minimise_fixed_sequential(mol: Chem.Mol, conf_ids, fixed_idx, its: int = 200) -> None:
...
def mmff_minimise_fixed_parallel(mol_original: Chem.Mol, conf_ids: List[int], fixed_idx: List[int], 
                               its: int = 200, n_workers: int = 1):
    ...
    if n_workers > 1 and len(conf_ids) > 1:
        logger.debug(f"Running MMFF minimization in parallel for {len(conf_ids)} conformers with {n_workers} workers.")
        tasks = []
        for cid in conf_ids:
        ...
        with ProcessPoolExecutor(max_workers=n_workers) as executor:
            ...
            for result in tqdm(results_iterator, total=len(tasks), desc="MMFF Min (Parallel)", disable=True):
                if result:
                    ...
                    if minimized_coords is not None:
                        ...
                        logger.warning(f"MMFF minimization failed or returned no coords for conformer ID {cid_original}.")
        ...
        for cid, new_coords_list in minimized_conformers_data.items():
            if cid < mol_original.GetNumConformers(): # Check if cid is valid
                ...
                if len(new_coords_list) == conformer_to_update.GetNumAtoms():
                    ...
                    logger.warning(f"Coordinate length mismatch for conformer {cid} after MMFF. Expected {conformer_to_update.GetNumAtoms()}, got {len(new_coords_list)}.")
            else:
                logger.warning(f"Conformer ID {cid} out of range after parallel MMFF.")
        logger.debug(f"Finished parallel MMFF minimization. Updated {updated_count}/{len(conf_ids)} conformers on original molecule.")

    else: # Sequential execution uses the original molecule directly
        logger.debug(f"Running MMFF minimization sequentially for {len(conf_ids)} conformers.")
        mp = rdForceFieldHelpers.MMFFGetMoleculeProperties(mol_original, mmffVariant="MMFF94s")
        if mp is None:
            logger.warning("MMFF parameters missing for sequential minimization, skipping.")
            return
        ...
        for cid in tqdm(conf_ids, desc="MMFF Min (Sequential)", disable=True):
            if cid < mol_original.GetNumConformers():
                ...
                if ff is None:
                    logger.warning(f"Could not get MMFF force field for conformer {cid} (sequential), skipping.")
                    continue
                ...
                try:
                    ...
                    logger.error(f"MMFF Minimization failed for conformer {cid} (sequential): {e_seq}")
            else:
...
def central_atom_embed(tgt: Chem.Mol, ref: Chem.Mol, n_conformers: int, n_workers: int) -> Optional[Chem.Mol]:
    ...
    logger.info(f"Central atom positioning: target atom {tgt_central} -> template atom {ref_central}")
...
def constrained_embed(tgt: Chem.Mol, ref: Chem.Mol, smarts: str, n_conformers: int = 100, 
                    n_workers: int = 1) -> Optional[Chem.Mol]:
    ...
    tgt_processed, tgt_had_metals, tgt_subs = detect_and_substitute_organometallic(tgt, "target")
    ref_processed, ref_had_metals, ref_subs = detect_and_substitute_organometallic(ref, "template")
    ...
    if not tgt_idxs or not ref_idxs or len(tgt_idxs) != len(ref_idxs) or len(tgt_idxs) < 3:
        if logger.isEnabledFor(logging.DEBUG):
            logger.debug(f"Invalid MCS match for constrained embedding. Target idx: {tgt_idxs}, Ref idx: {ref_idxs}. Proceeding with unconstrained embedding.")
    ...
    logger.debug(f"Attempting batched constrained embedding with {n_conformers} conformers")
    ...
    try:
        ...
        if all_conformers and all_conformers.GetNumConformers() > 0:
            logger.debug(f"Batched constrained embedding generated {all_conformers.GetNumConformers()} conformers")
            
            # MMFF minimization with fixed atoms
            conf_ids = list(range(all_conformers.GetNumConformers()))
            mmff_minimise_fixed_parallel(all_conformers, conf_ids, tgt_idxs, n_workers=n_workers)
            ...
            logger.debug("Batched constrained embedding failed, trying unconstrained fallback")
            
    except Exception as e:
        logger.debug(f"Batched constrained embedding exception: {str(e)}, trying unconstrained fallback")
    
    # Fallback if embedding fails
    if logger.isEnabledFor(logging.DEBUG):
        logger.debug("Constrained embedding failed, falling back to unconstrained embedding.")
    probe = Chem.AddHs(tgt_processed)
    return generate_conformers_batched(probe, n_conformers, n_workers)

def generate_conformers_batched(mol: Chem.Mol, n_conformers: int, n_workers: int) -> Optional[Chem.Mol]:
    ...
    while remaining > 0:
        ...
        try:
            ...
            logger.debug(f"Batch {batch_count}: generated {len(conf_ids)} conformers, {remaining} remaining")
    ...
    logger.debug(f"Batched generation complete: {result_mol.GetNumConformers()} total conformers")
    return result_mol if result_mol.GetNumConformers() > 0 else None

def generate_conformers_batched_constrained(mol: Chem.Mol, n_conformers: int, coord_map: Dict, n_workers: int) -> Optional[Chem.Mol]:
    ...
    if n_conformers <= CONFORMER_BATCH_SIZE:
        ...
        conf_ids = rdDistGeom.EmbedMultipleConfs(mol, n_conformers, ps, coordMap=coord_map)
    ...
    while remaining > 0:
        ...
        try:
            conf_ids = rdDistGeom.EmbedMultipleConfs(batch_mol, batch_size, ps, coordMap=coord_map)
            ...
            remaining -= len(conf_ids)
            logger.debug(f"Constrained batch {batch_count}: generated {len(conf_ids)} conformers, {remaining} remaining")
    ...
    logger.debug(f"Batched constrained generation complete: {result_mol.GetNumConformers()} total conformers")
    return result_mol if result_mol.GetNumConformers() > 0 else None
...
def transform_ligand(mob_pdb: str, lig: Chem.Mol, pid: str, ref_struct: AtomArray, 
    ...
    try:
        ...
        if not selected_ref_chains:
            ...
            logger.warning(f"Using fallback chain {selected_ref_chains[0]} for reference structure - may affect binding pocket alignment")
        ...
        if not selected_mob_chains:
            ...
            logger.warning(f"Using fallback chain {selected_mob_chains[0]} for mobile structure - may affect binding pocket alignment")
        ...
        for chain in selected_ref_chains:
            ...
            if len(chain_ca) > 0:
                ...
                logger.debug(f"Using {len(chain_ca)} CA atoms from reference chain {chain}")
        ...
        for chain in selected_mob_chains:
            ...
            if len(chain_ca) > 0:
                ...
                logger.debug(f"Using {len(chain_ca)} CA atoms from mobile chain {chain}")
        ...
        if len(ref_ca_atoms) == 1:
            ...
            try:
                ...
                logger.debug(f"Standard stacking failed for ref chains in {pid}, standardizing annotations: {str(e)}")
                try:
                    ...
                    if len(std_ref_ca_atoms) > 1:
                        ...
                        logger.warning(f"Using only first chain for reference in {pid} after standardization")
                except Exception as e2:
                    # Last resort: just use the first chain
                    logger.warning(f"Standardization failed for reference in {pid}, using only first chain: {str(e2)}")
                    ref_ca = ref_ca_atoms[0]
        ...
        if len(mob_ca_atoms) == 1:
            ...
            try:
                ...
                logger.debug(f"Standard stacking failed for mobile chains in {pid}, standardizing annotations: {str(e)}")
                try:
                    # If fails, standardize annotations and try again
                    ...
                    if len(std_mob_ca_atoms) > 1:
                        ...
                        logger.warning(f"Using only first chain for mobile in {pid} after standardization")
                except Exception as e2:
                    # Last resort: just use the first chain
                    logger.warning(f"Standardization failed for {pid}, using only first chain: {str(e2)}")
                    mob_ca = mob_ca_atoms[0]
                    # Update selected chains to match
                    selected_mob_chains = [selected_mob_chains[0]]
        ...
        if min(len(ref_ca), len(mob_ca)) < 3:
            logger.warning(f"Too few CA atoms for {pid}: ref={len(ref_ca)}, mob={len(mob_ca)}")
            return None
        ...
        try:
            ...
            logger.debug(f"Aligned {len(fixed_idx)} residues for {pid} using embedding-specified chains")
            ...
            try:
                ...
                logger.warning(f"Direct superimposition also failed for {pid}: {str(e2)}")
                return None
