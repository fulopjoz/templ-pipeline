...
class MoleculeSkipException(Exception):
    ...
    def __init__(
        self, reason: SkipReason, message: str, details: Optional[Dict[str, Any]] = None
    ):
        self.reason = reason
...
class ExecutionManager:
    ...
    def __init__(self):
        if not hasattr(self, "_initialized"):
            ...
            logger.info(
                f"Execution manager initialized with max_threads={self._max_threads}"
            )
    ...
    def get_safe_worker_count(
        self, requested_workers: int, task_type: str = "general"
    ) -> int:
    ...
    def get_skip_summary(self) -> Dict[str, Any]:
        ...
        summary = {
            ...
            "skip_counts_by_reason": {
                reason.value: count for reason, count in self._skip_counts.items()
            },
            "skip_rate_by_reason": {},
        ...
        if total_skipped > 0:
            for reason, count in self._skip_counts.items():
                summary["skip_rate_by_reason"][reason.value] = (
                    count / total_skipped
                ) * 100
    ...
    def get_execution_summary(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "total_processed": self._total_processed,
            "total_failed": self._total_failed,
            "success_rate": self._total_processed
            / max(1, self._total_processed + self._total_failed)
            * 100,
        }
    ...
    def save_execution_report(self, output_path: str) -> bool:
        ...
        try:
            report_data = {
                ...
                "detailed_skip_records": [
                    record.to_dict() for record in self.skipped_records
                ],
    ...
    def create_validation_skip_wrapper(self, validation_func):
        ...
        def wrapper(mol, mol_name="unknown", **kwargs):
            ...
            if not is_valid:
                ...
                raise MoleculeSkipException(
                    reason, message, {"molecule_name": mol_name}
                )
    ...
    def with_error_handling(self, func, molecule_id: str = "unknown"):
        ...
        def wrapper(*args, **kwargs):
            try:
                ...
                raise MoleculeSkipException(
                    SkipReason.OTHER, f"Unexpected error: {str(e)}"
                )
...
def raise_skip_exception(
    reason: SkipReason, message: str, details: Optional[Dict[str, Any]] = None
) -> None:
...
def create_robust_process_pool(
    max_workers: int, task_type: str = "general"
) -> RobustProcessPoolWrapper:
    ...
    logger.info(
        f"Creating robust process pool with {safe_workers} workers for {task_type}"
    )
