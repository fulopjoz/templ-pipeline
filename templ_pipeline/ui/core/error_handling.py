...
class ContextualErrorManager:
    ...
    def handle_error(
        ...
        if not user_message:
            user_message = self._generate_user_message(
                error_category, error_subtype, exception
            )
    ...
    def _generate_user_message(
        self, category: str, subtype: Optional[str], exception: Exception
    ) -> str:
        ...
        if "file" in exception_str and "size" in exception_str:
            ...
            return (
                "Not enough memory to complete this operation. Try a smaller molecule."
            )
    ...
    def _display_user_error(self, error_record: ErrorRecord, show_recovery: bool):
        ...
        if error_record.category in ["CRITICAL", "PIPELINE_ERROR"]:
            with st.expander("Need help?", expanded=False):
                ...
                st.write(
                    "This reference contains diagnostic information to help resolve the issue."
                )
...
def handle_molecular_error(
    exception: Exception, operation: str = "molecular processing"
) -> str:
...
def handle_pipeline_error(
    exception: Exception, operation: str = "pipeline execution"
) -> str:
