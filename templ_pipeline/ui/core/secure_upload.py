...
class SecureFileUploadHandler:
    ...
    def __init__(self, upload_dir: Optional[str] = None):
        ...
        try:
            ...
            logger.warning(f"Could not set restrictive permissions on {self.upload_dir}")
    
    def validate_and_save(self, uploaded_file, file_type: str, 
        ...
        if file_ext not in self.ALLOWED_EXTENSIONS:
            return False, f"File type '{file_ext}' not allowed. Supported: {', '.join(self.ALLOWED_EXTENSIONS)}", None
        ...
        if uploaded_file.size > max_size_bytes:
            return False, f"File exceeds {max_size_mb}MB limit (current: {uploaded_file.size / (1024*1024):.1f}MB)", None
        
        # Validate filename
        original_filename = uploaded_file.name
        if not self._is_safe_filename(original_filename):
            return False, "Invalid filename. Please use only alphanumeric characters, hyphens, and underscores.", None
        ...
        try:
            ...
            try:
                ...
                logger.warning(f"Could not set restrictive permissions on {secure_path}")
            
            logger.info(f"Securely saved file: {secure_filename} ({len(content)} bytes)")
            return True, f"File validated and saved ({len(content)} bytes)", str(secure_path)
    ...
    def _validate_content(self, content: bytes, file_ext: str) -> Tuple[bool, str]:
        ...
        try:
            ...
            if file_ext == '.pdb':
                ...
                has_atom_records = any(line.startswith(('ATOM', 'HETATM')) for line in lines[:100])
                ...
                if '$$$$' in text_content or 'M  END' in text_content or len(text_content.splitlines()) > 3:
                    # Basic structure indicators present
                    pass
                else:
                    return False, f"Invalid {file_ext} file - no molecular structure found"
                ...
                for line in lines[:10]:  # Check first 10 lines
                    if line.strip() and not smiles_pattern.match(line.strip().split()[0]):
...
def validate_file_secure(uploaded_file, file_type: str, max_size_mb: int = 5) -> Tuple[bool, str, Optional[str]]:
