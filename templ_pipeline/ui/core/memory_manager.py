...
class MolecularSessionManager:
    ...
    def store_molecule(
        self, key: str, mol: Any, metadata: Optional[Dict] = None
    ) -> bool:
        ...
        try:
            ...
            if mol_size < 50000:  # 50KB threshold for direct storage
                ...
                logger.info(
                    f"Large molecule {key} stored as SMILES only ({mol_size} bytes)"
                )
    ...
    def get_pose_results(self) -> Optional[Dict[str, Tuple[Any, Dict]]]:
        ...
        if not STREAMLIT_AVAILABLE:
            logger.debug("Streamlit not available for pose retrieval")
            return None

        # Check if we have pose references
        if "best_poses_refs" not in st.session_state:
            logger.debug("No best_poses_refs found in session state")
            return None
    ...
    def store_large_object(
        ...
        try:
            ...
            # Check if it's a poses dictionary (dict with tuple values containing molecules)
                ...
                if (
                    isinstance(sample_value, tuple)
                    and len(sample_value) == 2
                    and hasattr(sample_value[0], "ToBinary")
                ):
            ...
            try:
                ...
                if obj_size < 100000:  # 100KB threshold for direct storage
                    ...
                    logger.warning(
                        f"Very large object {key} ({obj_size} bytes) - storing reference only"
                    )
                    if STREAMLIT_AVAILABLE:
            ...
            try:
                ...
                logger.error(
                    f"Fallback storage also failed for {key}: {fallback_error}"
                )
                return False

            return False

    def _store_general_direct(
    ...
    def _store_general_compressed(
        ...
        try:
            ...
            logger.debug(
                f"Compressed general object {key}: {len(serialized)} -> {len(compressed)} bytes ({compression_ratio:.2%})"
            )
    ...
    def _store_compressed(self, key: str, mol: Any):
        ...
        try:
            ...
            logger.debug(
                f"Compressed {key}: {len(mol_binary)} -> {len(compressed)} bytes ({compression_ratio:.2%})"
            )
    ...
    def _cleanup_cache(self):
        ...
        logger.info(
            f"Cache cleanup completed: {initial_size / 1024 / 1024:.1f}MB -> {final_size / 1024 / 1024:.1f}MB in {cleanup_time:.2f}s"
        )
    ...
    def clear_cache(self, key_pattern: Optional[str] = None):
        ...
        if key_pattern:
            ...
            logger.info(
                f"Cleared {len(keys_to_remove)} cache entries matching '{key_pattern}'"
            )
        else:
            # Clear all caches
    ...
    def optimize_memory(self) -> Dict[str, Any]:
        ...
        logger.info(
            f"Memory optimization: {result['memory_saved_mb']:.1f}MB saved in {optimization_time:.2f}s"
        )
...
def store_molecule_efficient(
    key: str, mol: Any, metadata: Optional[Dict] = None
) -> bool:
