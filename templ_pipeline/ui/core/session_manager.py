...
class SessionManager:
    ...
    def set(self, key: str, value: Any, track_large: bool = None) -> None:
        ...
        if track_large and value is not None:
            self.memory_manager.store_large_object(key, value, {
    ...
    def clear(self, keys: Optional[List[str]] = None, preserve_core: bool = True) -> None:
        """Clear session state
        ...
        if keys:
            ...
            preserve_keys = {SESSION_KEYS["APP_INITIALIZED"], "session_start_time"} if preserve_core else set()
    ...
    def has_valid_input(self) -> bool:
        ...
        has_protein = bool(
            self.get(SESSION_KEYS["PROTEIN_PDB_ID"]) or 
            self.get(SESSION_KEYS["PROTEIN_FILE_PATH"]) or
            self.get(SESSION_KEYS["CUSTOM_TEMPLATES"])
        )
