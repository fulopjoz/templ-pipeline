...
def test_molecule_storage_and_retrieval():
    ...
    try:
        ...
        if retrieved_mol:
            print(
                f"✅ Successfully retrieved query molecule: {Chem.MolToSmiles(retrieved_mol)}"
            )
        else:
        ...
        if retrieved_template:
            print(
                f"✅ Successfully retrieved template molecule: {Chem.MolToSmiles(retrieved_template)}"
            )
        else:
        ...
        mcs_info_dict = {"smarts": mcs_smarts, "atom_count": 3, "valid_smarts": True}
