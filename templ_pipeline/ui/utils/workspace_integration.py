...
class UIWorkspaceIntegration:
    ...
    def get_secure_upload_handler(self):
        ...
        return SecureFileUploadHandler(workspace_manager=self.workspace_manager)
    ...
    def handle_file_upload(
        ...
        try:
            ...
            if success and file_path:
                ...
                logger.info(
                    f"File uploaded successfully: {uploaded_file.name} -> {file_path}"
                )
    ...
    def get_workspace_summary(self) -> Dict[str, Any]:
        ...
        if not self.workspace_manager:
            return {"enabled": False, "message": "Unified workspace not available"}
        ...
        try:
            with st.sidebar:
                ...
                with col1:
                    if st.button(
                        "📋 View Files",
                        help="Show workspace files",
                        use_container_width=True,
                    ):
                        st.session_state.show_workspace_files = (
                            not st.session_state.get("show_workspace_files", False)
                        )
                        st.rerun()
                ...
                if st.session_state.get("show_workspace_files", False):
                    ...
                    if output_files:
                        for file_info in output_files[:5]:  # Show max 5 recent files
                            ...
                            with col2:
                                success, content, filename = self.download_file(
                                    file_info["path"]
                                )
                                if success:
                                    st.download_button(
            ...
            for file_info in output_files:
                with st.expander(
                    ...
                    with col1:
                        ...
                        st.write(
                            f"**Modified:** {time.ctime(file_info['modified_time'])}"
                        )
...
def get_workspace_integration(
    session_manager, pipeline_service
) -> UIWorkspaceIntegration:
    """
