...
def create_all_conformers_sdf():
    ...
    try:
        ...
        logger.info(
            f"Retrieved all_ranked_poses from session manager: {type(all_ranked_poses)}"
        )
    except Exception as e:
        ...
        all_ranked_poses = st.session_state.get("all_ranked_poses")
        logger.info(
            f"Retrieved all_ranked_poses from session state: {type(all_ranked_poses)}"
        )
    ...
    for key in pose_related_keys:
        ...
        if value is not None:
            logger.info(
                f"DEBUG: Session key '{key}': type={type(value)}, length={len(value) if hasattr(value, '__len__') else 'N/A'}"
            )
    ...
    try:
        ...
        for rank, pose_data in enumerate(all_ranked_poses, 1):
            try:
                logger.debug(
                    f"Processing pose {rank}: {type(pose_data)}, length: {len(pose_data) if hasattr(pose_data, '__len__') else 'N/A'}"
                )
                ...
                if len(pose_data) == 3:
                    ...
                    logger.debug(
                        f"Pose {rank}: conf_id={conf_id}, scores={type(scores)}, mol={type(mol)}"
                    )
                else:
                    # Handle legacy format: (pose, scores, original_cid)
                    mol, scores, conf_id = pose_data
                    logger.debug(
                        f"Pose {rank} (legacy): mol={type(mol)}, scores={type(scores)}, conf_id={conf_id}"
                    )

                # Skip invalid poses
                if mol is None or not hasattr(mol, "ToBinary"):
                    logger.warning(
                        f"Skipping invalid pose at rank {rank}: mol={type(mol)}"
                    )
                    continue
            ...
            try:
                template_info = (
                    session.get("template_info")
                    if "session" in locals()
                    else st.session_state.get("template_info")
                )
                if template_info:
                    ...
                    mol_copy.SetProp(
                        "Template_Index", str(template_info.get("index", 0))
                    )
                    mol_copy.SetProp(
                        "Total_Templates", str(template_info.get("total_templates", 1))
                    )
                    mol_copy.SetProp(
                        "Atoms_Matched", str(template_info.get("atoms_matched", 0))
                    )
            except Exception as e:
