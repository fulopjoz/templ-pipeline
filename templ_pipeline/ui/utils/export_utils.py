...
def create_all_conformers_sdf():
    ...
    if (
        not hasattr(st.session_state, "all_ranked_poses")
        or not st.session_state.all_ranked_poses
    ):
    ...
    try:
        ...
        for rank, (pose, scores, original_cid) in enumerate(
            st.session_state.all_ranked_poses, 1
        ):
            ...
            if (
                ...
                if "mcs_smarts" in st.session_state.template_info:
                    mol_copy.SetProp(
                        "MCS_SMARTS", st.session_state.template_info["mcs_smarts"]
                    )

                # Add additional template metadata
                mol_copy.SetProp(
                    "Template_Index",
                    str(st.session_state.template_info.get("index", 0)),
                )
                mol_copy.SetProp(
                    "Total_Templates",
                    str(st.session_state.template_info.get("total_templates", 1)),
                )
                mol_copy.SetProp(
                    "Atoms_Matched",
                    str(st.session_state.template_info.get("atoms_matched", 0)),
                )
