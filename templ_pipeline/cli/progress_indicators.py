...
def progress_context(
    ...
    style = _determine_progress_style(verbosity, experience_level, show_progress_bars, total_items)
        ...
        def spin():
            ...
            while self.is_running:
                if time.time() - self.last_update_time > 0.1:  # Update every 100ms
                    sys.stdout.write(f"\r{spinner_chars[i % len(spinner_chars)]} {self.description}")
                    sys.stdout.flush()
                    i += 1
                time.sleep(0.1)
            sys.stdout.write("\r" + " " * (len(self.description) + 5) + "\r")  # Clear line
            sys.stdout.flush()
    ...
    def update(self, increment: int = 1, message: Optional[str] = None):
        ...
        if self.style == ProgressStyle.PROGRESS_BAR and self.progress_bar:
            ...
            if (current_time - self.last_update_time > 5.0 or 
                (self.total_items and self.current_item % max(1, self.total_items // 10) == 0)):
                ...
                if self.total_items:
                    ...
                    print(f"PROGRESS: {self.current_item}/{self.total_items} ({progress_pct:.1f}%) - {elapsed:.1f}s elapsed{eta_str}")
                else:
                    rate = self.current_item / elapsed if elapsed > 0 else 0
                    rate_str = f" ({rate:.1f} items/s)" if self.show_rate else ""
                    print(f"PROCESSED: {self.current_item} items{rate_str} - {elapsed:.1f}s elapsed")
    ...
    def finish(self, success_message: Optional[str] = None):
        ...
        if self.style not in [ProgressStyle.MINIMAL, ProgressStyle.SILENT]:
            if success_message:
                ...
                rate = self.current_item / elapsed if elapsed > 0 and self.current_item > 0 else 0
                rate_str = f" ({rate:.1f} items/s)" if self.show_rate and rate > 0 else ""
...
class SimpleProgressBar:
    ...
    def _display(self):
        ...
        print(f"\r{self.description} |{bar}| {self.current}/{self.total} ({progress*100:.1f}%) "
    ...
    try:
        ...
        if 'gpu_info' in status:
            for i, gpu in enumerate(status['gpu_info']):
                print(f"   GPU {i}: {gpu.get('utilization', 'Unknown')}% util, "
                      f"{gpu.get('memory_percent', 'Unknown')}% memory")
        
    except ImportError:
        logger.debug("Hardware monitoring not available")

def estimate_operation_time(operation_type: OperationType, context: Dict[str, Any]) -> Optional[str]:
