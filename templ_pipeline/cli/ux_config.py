...
class UserPreferences:
    ...
    def from_dict(cls, data: Dict) -> 'UserPreferences':
        ...
        return cls(
            ...
            preferred_output_format=data.get("preferred_output_format", "user-friendly"),
...
class UsagePattern:
    ...
    def calculate_experience_level(self) -> ExperienceLevel:
        ...
        if self.total_commands < 5:
            ...
            advanced_usage = sum(self.command_usage_count.get(cmd, 0) for cmd in advanced_commands)
...
class TEMPLUXConfig:
    ...
    def _setup_argument_groups(self):
        ...
        self.argument_groups = {
            ...
            "essential": [
                ArgumentGroup("core_input", ArgumentComplexity.ESSENTIAL,
                            ["protein-file", "protein-pdb-id", "ligand-smiles", "ligand-file"],
                            "Core input options (always shown)"),
                ArgumentGroup("basic_control", ArgumentComplexity.ESSENTIAL,
            ...
            "common": [
                ArgumentGroup("template_control", ArgumentComplexity.COMMON,
                            ["num-templates", "num-conformers", "template-pdb"],
                            "Template and conformer settings"),
                ArgumentGroup("performance", ArgumentComplexity.COMMON,
            ...
            "advanced": [
                ArgumentGroup("fine_tuning", ArgumentComplexity.ADVANCED,
                            ["similarity-threshold", "exclude-uniprot-file", "chain", "run-id"],
                            "Fine-tuning and filtering options"),
                ArgumentGroup("output_control", ArgumentComplexity.ADVANCED,
            ...
            "debug": [
                ArgumentGroup("debugging", ArgumentComplexity.DEBUG,
                            ["log-level", "verbose"],
    ...
    def get_arguments_for_user_level(self, user_level: Optional[ExperienceLevel] = None) -> Set[str]:
    ...
    def should_show_performance_hints(self) -> bool:
        ...
        return self.preferences.show_performance_hints and \
               self.get_effective_experience_level() != ExperienceLevel.BEGINNER
    
    def get_smart_defaults(self, command: str, context: Dict[str, Any]) -> Dict[str, Any]:
    ...
    def track_error(self, error_context: str):
        ...
        if len(self.usage_patterns.error_patterns) > 50:
            self.usage_patterns.error_patterns = self.usage_patterns.error_patterns[-50:]
    
    def get_contextual_help_hints(self, command: str, partial_args: Dict[str, Any]) -> List[str]:
        """Get contextual help hints based on current context and user patterns."""
        hints = []
        
        # Beginner-specific hints
        if self.get_effective_experience_level() == ExperienceLevel.BEGINNER:
            if command == "run" and not partial_args.get("ligand-smiles") and not partial_args.get("ligand-file"):
                hints.append("TIP: You need either --ligand-smiles or --ligand-file to specify your query molecule")
            
            if command == "run" and not partial_args.get("protein-file") and not partial_args.get("protein-pdb-id"):
                hints.append("TIP: You need either --protein-file or --protein-pdb-id to specify your target protein")
        
        # Performance hints
        if self.should_show_performance_hints():
            if command in ["run", "generate-poses"] and not partial_args.get("workers"):
                hints.append(f"PERFORMANCE: Consider using --workers {self.get_smart_defaults(command, {})['workers']} for faster processing")
...
def configure_logging_for_verbosity(verbosity: VerbosityLevel, logger_name: str = "templ-cli"):
