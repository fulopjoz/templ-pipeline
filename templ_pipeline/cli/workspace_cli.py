...
def list_workspaces(workspace_root: str = "workspace") -> List[Dict[str, Any]]:
    ...
    for run_dir in workspace_path.glob("run_*"):
        ...
        try:
            ...
            # Count files by type
            temp_files = len(list((run_dir / "temp").rglob('*'))) if (run_dir / "temp").exists() else 0
            output_files = len(list((run_dir / "output").rglob('*'))) if (run_dir / "output").exists() else 0
...
def display_workspace_summary(workspace_root: str = "workspace"):
    ...
    for workspace in workspaces:
        ...
        print(f"    Files: {workspace['temp_files']} temp, {workspace['output_files']} output")
...
def cleanup_workspaces(
    ...
    if dry_run:
        ...
        for candidate in candidates:
            print(f"  - {Path(candidate['path']).name}: {candidate['size_mb']:.1f} MB (age: {candidate['age_days']:.1f} days)")
...
Examples:
  ...
  %(prog)s summary                        # Show workspace summary
    ...
    parser.add_argument(
        "--run-id",
        help="Custom run ID for test workspace creation"
    )
        ...
        if args.command == "list":
            ...
            if args.json:
                ...
                if workspaces:
                    ...
                    for w in workspaces:
                        print(f"  {w['run_id']} - {w['size_mb']:.1f}MB - {w['age_hours']:.1f}h old")
                else:
