...
def run_example(output_dir="example_output"):
    ...
    template_mols = []
    template_smiles = ['CCOC', 'CCCCO', 'CC(O)C']  # Methyl ethyl ether, butanol, isopropanol
    ...
    confs = constrained_embed(query_mol, template_mol, smarts, n_conformers=10, n_workers=2)
    ...
    for metric, (pose, scores) in best_poses.items():
        ...
        logger.info(f"RMSD for {metric} pose: {rms:.2f} Å (score: {scores[metric]:.3f})")
