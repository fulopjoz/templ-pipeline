...
def find_templates_for_target(
    ...
    if output_dir:
        try:
            ...
            visualize_templates(target_id, target_embedding, neighbors, manager, output_dir)
        except Exception as e:
...
def visualize_templates(
    ...
    plt.axvline(x=min(similarities), color='r', linestyle='--', 
...
def generate_report(
    ...
    with open(report_path, 'w') as f:
        ...
        if templates:
            f.write(f"Similarity range: {templates[-1][1]:.4f} - {templates[0][1]:.4f}\n\n")
...
def main():
    ...
    parser.add_argument(
        "--target", 
        required=True,
        help="Target PDB ID or path to PDB file"
    )
    parser.add_argument(
        "--embeddings", 
        default=None,
