...
try:
    ...
    raise RuntimeError("Required dependencies not available - please check templ_pipeline/requirements.txt") from e
...
class BenchmarkRunner:
    ...
    def _setup_components(self):
        ...
        if memory_status["critical"]:
            self.log.error(f"Critical memory usage at startup: {memory_status['memory_gb']:.1f}GB")
            self.pipeline = None
            return
        
        try:
            # Use correct subdirectory structure for embeddings
            embedding_path = self.data_dir / "embeddings" / "protein_embeddings_base.npz"
            ...
            self.log.debug(f"Initializing TEMPL pipeline with data_dir: {self.data_dir}")
            ...
            if embedding_path.exists():
                ...
                if alt_embedding_path.exists():
                    ...
                    self.log.warning("No embeddings found, initializing pipeline without embeddings")
                    self.pipeline = TEMPLPipeline(output_dir=str(output_dir))
                
            self.log.info(f"TEMPL pipeline initialized with {memory_status['memory_gb']:.1f}GB memory usage")
            
        except Exception as e:
            self.log.error(f"Pipeline initialization failed: {e}")
            self.pipeline = None
    
    def _load_ligand_data_from_sdf(self, pdb_id: str) -> Tuple[Optional[str], Optional["Chem.Mol"]]:
    ...
    def _calculate_rmsd_to_crystal(self, pose_mol: "Chem.Mol", crystal_mol: "Chem.Mol") -> float:
    ...
    def run_single_target(self, params: BenchmarkParams) -> BenchmarkResult:
        ...
        if memory_status["warning"]:
            self.log.warning(f"High memory usage for {params.target_pdb}: {memory_status['memory_gb']:.1f}GB")
        ...
        try:
            ...
            ligand_smiles, crystal_mol = self._load_ligand_data_from_sdf(params.target_pdb)
            ...
            try:
                ...
                if not is_valid:
                    self.log.debug(f"Skipping {params.target_pdb} due to crystal quality: {msg}")
            ...
            memory_status = monitor_memory_usage()
            if memory_status["critical"]:
                raise RuntimeError(f"Critical memory usage before pipeline: {memory_status['memory_gb']:.1f}GB")
            
            # Run TEMPL pipeline with memory-safe settings
            try:
                self.log.info(f"Starting pipeline execution for {params.target_pdb}")
                self.log.debug(f"Pipeline params: templates={params.template_knn}, conformers={params.n_conformers}, workers={params.internal_workers}")
                ...
                self.log.debug(f"Pipeline result keys: {list(pipeline_result.keys()) if isinstance(pipeline_result, dict) else 'Not a dict'}")
                
            except Exception as pipeline_error:
                self.log.error(f"Pipeline execution failed for {params.target_pdb}: {pipeline_error}")
            ...
            if not isinstance(pipeline_result, dict):
                raise ValueError(f"Pipeline returned invalid result type: {type(pipeline_result)}")
            ...
            self.log.debug(f"RMSD calculation result for {params.target_pdb}: {len(rmsd_values)} metrics calculated")
            if rmsd_values:
                for metric, values in rmsd_values.items():
                    self.log.debug(f"  {metric}: RMSD={values.get('rmsd', 'N/A'):.3f}, Score={values.get('score', 'N/A'):.3f}")
            else:
                self.log.warning(f"No RMSD values calculated for {params.target_pdb} - checking pipeline result structure")
                self.log.debug(f"Pipeline result type: {type(pipeline_result)}")
                if isinstance(pipeline_result, dict):
                    self.log.debug(f"Pipeline result keys: {list(pipeline_result.keys())}")
                    if 'poses' in pipeline_result:
                        poses = pipeline_result['poses']
                        self.log.debug(f"Poses type: {type(poses)}, length: {len(poses) if poses else 0}")
                        if poses:
                            for k, v in poses.items():
                                self.log.debug(f"  Pose {k}: type={type(v)}, valid={v is not None}")
            ...
            if final_memory["warning"]:
                self.log.warning(f"High memory after processing {params.target_pdb}: {final_memory['memory_gb']:.1f}GB")
            ...
            if rmsd_values:
                self.log.info(f"TEMPL completed for {params.target_pdb} in {runtime:.1f}s")
                return BenchmarkResult(
                    success=True,
                ...
                self.log.warning(f"TEMPL completed for {params.target_pdb} but no RMSD values")
                return BenchmarkResult(
                    success=False,
                    rmsd_values={},
                    runtime=runtime,
    ...
    def _get_protein_file(self, pdb_id: str) -> str:
        ...
        raise FileNotFoundError(f"Protein file not found for {pdb_id} in any of the search paths")
    
    def _extract_and_calculate_rmsd(self, pipeline_result: Dict, crystal_mol: Chem.Mol) -> Dict[str, Dict[str, float]]:
        ...
        try:
            ...
            for metric, pose_data in poses.items():
                try:
                    ...
                    if not isinstance(pose_data, tuple) or len(pose_data) != 2:
                        self.log.warning(f"Invalid pose data format for metric {metric}: expected tuple of length 2, got {type(pose_data)}")
                        continue
                    ...
                    if pose_mol.GetNumConformers() == 0:
                        self.log.warning(f"Pose molecule for {metric} has no conformers")
                        continue
                    ...
                    try:
                        ...
                        if pose_no_h.GetNumAtoms() == 0:
                            self.log.warning(f"Pose molecule for {metric} has no heavy atoms")
                            continue
                            
                        rmsd = self._calculate_rmsd_to_crystal(pose_no_h, crystal_ref)
                        
                        if rmsd is None or np.isnan(rmsd):
                            self.log.warning(f"RMSD calculation returned invalid value for {metric}")
                            continue
                    ...
                    try:
                        if isinstance(scores_dict, dict):
                            if metric in scores_dict:
                                ...
                                self.log.warning(f"No score found for metric {metric} in scores dict: {scores_dict.keys()}")
                        else:
                            self.log.warning(f"Scores dict for {metric} is not a dictionary: {type(scores_dict)}")
                    ...
                    rmsd_values[key_name] = {
                        "rmsd": float(rmsd),
                        "score": float(score)
                    }
                    self.log.debug(f"Calculated RMSD for {metric}: {rmsd:.3f}Å, score: {score:.3f}")
            ...
            if rmsd_values:
                self.log.info(f"Successfully calculated RMSD for {len(rmsd_values)} metrics")
            else:
