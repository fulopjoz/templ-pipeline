...
CUSTOM_SPLITS_DIR = f"{DATA_DIR}/time_splits_ghrepo" # Directory for custom PDB ID lists
...
OUTPUT_DIR_CUSTOM = "mcs_bench/output_custom"  # Fixed: Use correct relative path from mcs/ directory
    ...
    try:
        ...
        if mol and mol.HasProp("mcs_smarts"):
            return {
                ...
                "atom_count": int(mol.GetProp("mcs_atom_count")) if mol.HasProp("mcs_atom_count") else 0,
                "bond_count": int(mol.GetProp("mcs_bond_count")) if mol.HasProp("mcs_bond_count") else 0,
                "similarity_score": float(mol.GetProp("mcs_similarity_score")) if mol.HasProp("mcs_similarity_score") else 0.0,
                "query_atoms": mol.GetProp("mcs_query_atoms") if mol.HasProp("mcs_query_atoms") else "",
                "template_atoms": mol.GetProp("mcs_template_atoms") if mol.HasProp("mcs_template_atoms") else ""
            }
...
def analyze_mcs_patterns(results_data: Dict) -> Dict:
    ...
    for result in results_data.get("results", {}).values():
        if result.get("success") and result.get("mcs_info"):
            ...
            mcs_analysis["atom_count_distribution"].append(mcs_info.get("atom_count", 0))
            mcs_analysis["bond_count_distribution"].append(mcs_info.get("bond_count", 0))
            mcs_analysis["similarity_score_distribution"].append(mcs_info.get("similarity_score", 0.0))
    
    # Calculate statistics
    mcs_analysis["statistics"] = {
        "total_unique_patterns": len(mcs_analysis["unique_smarts_patterns"]),
        "avg_atom_count": np.mean(mcs_analysis["atom_count_distribution"]) if mcs_analysis["atom_count_distribution"] else 0,
        "avg_bond_count": np.mean(mcs_analysis["bond_count_distribution"]) if mcs_analysis["bond_count_distribution"] else 0,
        "avg_similarity_score": np.mean(mcs_analysis["similarity_score_distribution"]) if mcs_analysis["similarity_score_distribution"] else 0,
        "most_common_patterns": dict(sorted(mcs_analysis["pattern_frequency"].items(), key=lambda x: x[1], reverse=True)[:10])
    }
    
    return mcs_analysis

def save_enhanced_results(results_data: Dict, output_file: str, benchmark_name: str, split_name: str, organized_dirs: Optional[Dict[str, str]] = None) -> None:
    ...
    if organized_dirs:
        ...
        mcs_analysis_file = os.path.join(organized_dirs["mcs_patterns"], f"{split_name}_mcs_analysis.json")
        with open(mcs_analysis_file, 'w') as f:
            # Convert sets to lists for JSON serialization
            mcs_analysis_serializable = dict(mcs_analysis)
            mcs_analysis_serializable["unique_smarts_patterns"] = list(mcs_analysis["unique_smarts_patterns"])
        ...
        if "experiment_dir" in organized_dirs:
            ...
            experiment_results_file = os.path.join(organized_dirs["results"], f"results_{split_name}_{BENCHMARK_TIMESTAMP}.json")
        else:
            # Fallback to organized results subdirectory
            experiment_results_file = os.path.join(organized_dirs["results"], f"{split_name}_detailed_results.json")
        ...
        mcs_analysis_file = os.path.join(organized_dirs_fallback["mcs_patterns"], f"{split_name}_mcs_analysis.json")
        with open(mcs_analysis_file, 'w') as f:
            # Convert sets to lists for JSON serialization
            mcs_analysis_serializable = dict(mcs_analysis)
            mcs_analysis_serializable["unique_smarts_patterns"] = list(mcs_analysis["unique_smarts_patterns"])
    ...
    successful = sum(1 for r in results_data.get("results", {}).values() if r.get("success"))
    mcs_found = sum(1 for r in results_data.get("results", {}).values() if r.get("mcs_found"))
...
def extract_ca_rmsd_data(experiment_output_dir: str, target_pdb: str, output_files: Dict[str, Optional[str]]) -> Dict[str, Any]:
    ...
    try:
        if output_files.get("alignment_report"):
            ca_data_tracking = extract_ca_rmsd_from_alignment_report(output_files["alignment_report"])
            if ca_data_tracking:
            ...
            if mol.HasProp("ca_rmsd"):
                try:
                    ...
                    if mol.HasProp("aligned_residues_count"):
                        ca_data["matched_residues"] = int(mol.GetProp("aligned_residues_count"))
                    ...
                    if mol.HasProp("embedding_similarity"):
                        ca_data["embedding_similarity"] = float(mol.GetProp("embedding_similarity"))
        ...
        if template_info.get("ca_rmsd"):
            try:
                ...
                if template_info.get("aligned_residues_count"):
                    ca_data["matched_residues"] = int(template_info["aligned_residues_count"])
                ...
                if template_info.get("embedding_similarity"):
                    ca_data["embedding_similarity"] = float(template_info["embedding_similarity"])
    ...
    try:
        ...
        for log_entry in detailed_logs:
            if (log_entry.get("success") and 
                log_entry.get("stage") == "superimpose_homologs" and 
                log_entry.get("ca_rmsd") is not None):
                ...
                try:
                    ...
                    if log_entry.get("alignment_coverage_ref"):
                        ca_data["alignment_coverage"] = float(log_entry["alignment_coverage_ref"])
...
def run_pipeline(
    ...
    embedding_cache_dir: Optional[str] = None, # New parameter for custom cache directory
    ...
    if n_conformers > base_conformers:
        additional_time = (n_conformers - base_conformers) * additional_seconds_per_conformer
        adjusted_timeout = pipeline_timeout + additional_time
        log.debug(f"Scaling timeout for {n_conformers} conformers: {pipeline_timeout}s + {additional_time}s = {adjusted_timeout}s")
    else:
    ...
    cmd = [
        ...
        "--template-knn", str(template_knn),
        "--output-dir", experiment_output_dir,
        "--log-level", log_level,
        "--ca-rmsd-threshold", str(ca_rmsd_threshold)
    ]
    ...
    if template_pool_pdb_ids_file:
        cmd.extend(["--template-pdb-ids-file", template_pool_pdb_ids_file, "--enable-pdb-filtering"])
    ...
    try:
        ...
        if result.returncode == 0:
            ...
            log.info(f"Pipeline completed successfully for {target_pdb} in {runtime:.1f}s (timeout was {adjusted_timeout}s)")
            ...
            try:
                ...
                if output_files.get("structured_json"):
                    rmsd_results = extract_rmsd_from_json_structured(output_files["structured_json"])
                    if rmsd_results:
                        result_data["rmsd_data"] = rmsd_results
                        log.debug(f"Extracted RMSD data from JSON for {target_pdb}: {rmsd_results}")
                ...
                if not rmsd_results:
                    ...
                    if rmsd_results:
                        result_data["rmsd_data"] = rmsd_results
                        log.debug(f"Extracted RMSD data from stdout for {target_pdb}: {rmsd_results}")
                    else:
                        log.warning(f"No RMSD values parsed from stdout for {target_pdb}. Trying SDF fallback.")
                        
                        # 3. Fall back to SDF parsing if both JSON and stdout failed
                        if output_files.get("poses_sdf"):
                            rmsd_results = extract_rmsd_from_sdf(output_files["poses_sdf"])
                            if rmsd_results:
                                result_data["rmsd_data"] = rmsd_results
                                log.info(f"Successfully extracted RMSD from SDF for {target_pdb}")
                            else:
                                log.warning(f"No RMSD found in SDF either for {target_pdb}")
                        else:
            ...
            try:
                ...
                if output_files.get("all_poses_sdf"):
                    ...
                    log.debug(f"Extracted MCS info for {target_pdb}: poses={result_data['pose_count']}")
            except Exception as e:
                log.warning(f"Failed to extract MCS info for {target_pdb}: {e}")
            
            # Extract CA RMSD data from multiple sources
            try:
                ca_rmsd_data = extract_ca_rmsd_data(experiment_output_dir, target_pdb, output_files)
                if ca_rmsd_data:
                    result_data["ca_rmsd_data"] = ca_rmsd_data
                    ca_rmsd_value = ca_rmsd_data.get('ca_rmsd', 'N/A')
                    if isinstance(ca_rmsd_value, (int, float)):
                        log.info(f"Successfully extracted CA RMSD data for {target_pdb}: CA RMSD={ca_rmsd_value:.3f}Å")
                    else:
                        log.info(f"Successfully extracted CA RMSD data for {target_pdb}: CA RMSD={ca_rmsd_value}")
                else:
                    ...
                    if template_file and os.path.exists(template_file):
                        ...
                        if ca_data_template:
                            ...
                            if isinstance(ca_rmsd_value, (int, float)):
                                log.info(f"Extracted CA RMSD from template SDF for {target_pdb}: {ca_rmsd_value:.3f}Å")
                            else:
                                log.info(f"Extracted CA RMSD from template SDF for {target_pdb}: {ca_rmsd_value}")
            except Exception as e:
                log.warning(f"Failed to extract CA RMSD data for {target_pdb}: {e}")
        else:
            log.error(f"Pipeline failed for {target_pdb} with exit code {result.returncode}")
            result_data["error_message"] = f"Exit code {result.returncode}: {result.stderr.strip()}"
            if result.stderr:
        ...
        runtime = time.time() - start_time
        result_data["runtime_seconds"] = runtime
        result_data["error_message"] = f"Pipeline timed out after {adjusted_timeout} seconds"
        log.error(f"Pipeline timeout for {target_pdb} after {adjusted_timeout}s (requested {n_conformers} conformers)")
...
def evaluate_split(
    ...
    log.info(f"Created experiment directory for {split_name_key} split: {experiment_output_dir}")
    ...
    if max_pdbs_to_process and max_pdbs_to_process > 0:
        ...
        log.info(f"Limiting {split_name_key} evaluation to {len(pdbs_to_run)} PDBs for testing.")
    ...
    if n_workers > 1 and internal_pipeline_workers > 1:
        log.warning(f"Both benchmark n_workers ({n_workers}) and pipeline internal workers ({internal_pipeline_workers}) > 1. Setting pipeline internal workers to 1 to prevent oversubscription.")
    ...
    if n_workers > 1 and len(pdbs_to_run) > 1:
        ...
        with ProcessPoolExecutor(max_workers=n_workers, mp_context=mp_context) as executor:
            ...
            for fut in tqdm(as_completed(futures), total=len(futures), desc=f"Evaluating {split_name_key}"):
                pdb_id = futures[fut]
                try:
                    split_run_results[pdb_id] = fut.result()
                except Exception as e_future:
                    log.error(f"Exception from future for {pdb_id}: {str(e_future)}")
                    split_run_results[pdb_id] = {"success": False, "error": str(e_future), "rmsd_values": {}}
    else:
        log.info(f"Running {split_name_key} sequentially (n_workers={n_workers}, pdbs_to_run={len(pdbs_to_run)})..." )
        for pdb_id in tqdm(pdbs_to_run, desc=f"Evaluating {split_name_key} (sequential)"):
            split_run_results[pdb_id] = run_pipeline(
                pdb_id, experiment_output_dir, n_conformers, template_knn,
                similarity_threshold, pipeline_timeout, multiprocessing_pipeline,
                template_pool_file_for_split, log_level, use_embedding_cache, embedding_cache_dir, batch_embedding, max_batch_size,
                os.cpu_count() or 1, # Use all CPUs for pipeline if running sequentially
                ca_rmsd_threshold,  # Pass the CA RMSD threshold
                True  # Pass the save_enhanced_output flag
            )
...
def calculate_metrics(split_results_data: Dict) -> Dict:
    ...
    if not pdb_results: return {"total": 0, "successful": 0}
    ...
    for pdb_id, r_data in pdb_results.items():
        if r_data.get("success"):
            ...
            if r_data.get("runtime_seconds") is not None: metrics["runtimes"].append(r_data["runtime_seconds"])
            ...
            if r_data.get("ca_rmsd_data"):
                ...
                if "matched_residues" in ca_data:
                    metrics["ca_rmsd_matched_residues"].append(ca_data["matched_residues"])
            ...
            if r_data.get("rmsd_data"):
                for metric_key, values_dict in r_data["rmsd_data"].items():
                    ...
                    if score is not None: metrics["all_scores"][metric_key].append(score)
    ...
    if metrics["ca_rmsd_values"]:
        ...
        if metrics["ca_rmsd_matched_residues"]:
            metrics["ca_rmsd_stats"]["avg_matched_residues"] = np.mean(metrics["ca_rmsd_matched_residues"])
        if metrics["ca_rmsd_coverage"]:
            metrics["ca_rmsd_stats"]["avg_coverage"] = np.mean(metrics["ca_rmsd_coverage"])
        ...
        for metric_key, rmsd_list in metrics["all_rmsds"].items():
            if len(rmsd_list) > 5 and len(metrics["ca_rmsd_values"]) >= len(rmsd_list):
                ...
                try:
                    ...
                    metrics["ca_rmsd_stats"][f"correlation_with_{metric_key}"] = correlation
                except Exception as e:
                    log.warning(f"Could not calculate correlation between CA RMSD and {metric_key} RMSD: {e}")
...
def plot_rmsd_distribution(split_results_data: Dict, split_name_key: str, output_plot_dir: str, histogram_bins: int = 50) -> None:
    pdb_results = split_results_data.get("results", {})
    if not pdb_results: return
    rmsds_by_metric = defaultdict(list)
    for r_data in pdb_results.values():
        if r_data.get("success") and r_data.get("rmsd_data"):
            for metric_key, values_dict in r_data["rmsd_data"].items():
                if values_dict.get("rmsd") is not None: rmsds_by_metric[metric_key].append(values_dict["rmsd"])
    if not rmsds_by_metric: return
    ...
    for metric_key, values in rmsds_by_metric.items():
        ...
        plt.hist(values, bins=histogram_bins, alpha=0.8, color=color, edgecolor='black', linewidth=1.2, zorder=2)
        plt.axvline(x=2.0, color='#e67e22', linestyle='--', linewidth=2.5, label='2Å', zorder=1)
        plt.axvline(x=5.0, color='#9b59b6', linestyle='-.', linewidth=2.5, label='5Å', zorder=1)
        plt.xlabel('RMSD (Å)', fontsize=14); plt.ylabel('Count', fontsize=14)
        ...
        plt.text(0.95, 0.95, f'Mean: {mean_val:.2f}Å\nMedian: {median_val:.2f}Å\nMin: {min_val:.2f}Å\nMax: {max_val:.2f}Å\n>50Å: {outliers_50}', transform=plt.gca().transAxes, ha='right', va='top', bbox=dict(facecolor='white', alpha=0.8, edgecolor='black'), fontsize=12)
        s2A,s5A = (sum(1 for v in values if v <= t) / len(values) * 100 for t in [2.0,5.0])
        plt.text(0.05, 0.95, f'≤2Å: {s2A:.1f}%\n≤5Å: {s5A:.1f}%', transform=plt.gca().transAxes, ha='left', va='top', bbox=dict(facecolor='white', alpha=0.8, edgecolor='black'), fontsize=12)
        plt.xlim(left=0, right=max(10, min(max(values) + 1 if values else 10, 20)))
        plt.legend(loc='center right', frameon=True, fontsize=12, bbox_to_anchor=(0.98, 0.5))
        plt.grid(True, linestyle='--', alpha=0.7, zorder=0); plt.gca().set_axisbelow(True); plt.gca().set_facecolor('#f9f9f9'); plt.tight_layout()
        ts = datetime.now().strftime("%Y%m%d_%H%M%S")
        plot_file = os.path.join(plot_dir, f"rmsd_dist_{split_name_key}_{metric_key}_{ts}.png")
        plt.savefig(plot_file, dpi=300); log.info(f"Saved RMSD plot to {plot_file}"); plt.close()

def plot_ca_rmsd_distribution(split_results_data: Dict, split_name_key: str, output_plot_dir: str, histogram_bins: int = 50) -> None:
    ...
    ca_rmsd_values = []
    for r_data in pdb_results.values():
        if r_data.get("success") and r_data.get("ca_rmsd_data") and "ca_rmsd" in r_data["ca_rmsd_data"]:
            ca_rmsd_values.append(r_data["ca_rmsd_data"]["ca_rmsd"])
    
    if not ca_rmsd_values: 
        log.warning(f"No CA RMSD values found for {split_name_key}, skipping CA RMSD plot")
        return
    ...
    plt.hist(ca_rmsd_values, bins=histogram_bins, alpha=0.8, color=color, edgecolor='black', linewidth=1.2, zorder=2)
    
    # Add a vertical line at the threshold value (assume 10.0 if not otherwise known)
    threshold = 10.0  # Default threshold
    plt.axvline(x=threshold, color='#e74c3c', linestyle='--', linewidth=2.5, label=f'{threshold}Å Threshold', zorder=1)
    
    plt.xlabel('C-alpha RMSD (Å)', fontsize=14)
    plt.ylabel('Count', fontsize=14)
    plt.title(f'C-alpha RMSD Distribution for {split_name_key}', fontsize=16, fontweight='bold')
    ...
    pct_below = sum(1 for v in ca_rmsd_values if v <= threshold) / len(ca_rmsd_values) * 100
    plt.text(0.05, 0.95, f'≤{threshold}Å: {pct_below:.1f}%', 
            transform=plt.gca().transAxes, ha='left', va='top', 
            bbox=dict(facecolor='white', alpha=0.8, edgecolor='black'), fontsize=12)
    
    plt.xlim(left=0, right=max(threshold * 1.5, min(max(ca_rmsd_values) + 1 if ca_rmsd_values else threshold * 1.5, threshold * 2)))
    plt.legend(loc='center right', frameon=True, fontsize=12, bbox_to_anchor=(0.98, 0.5))
...
def plot_ca_rmsd_vs_ligand_rmsd(split_results_data: Dict, split_name_key: str, output_plot_dir: str) -> None:
    ...
    data_pairs = defaultdict(list)  # Dict mapping metric -> list of (ca_rmsd, ligand_rmsd) pairs
    
    for r_data in pdb_results.values():
        if not r_data.get("success") or not r_data.get("ca_rmsd_data") or "ca_rmsd" not in r_data["ca_rmsd_data"]:
            continue
    ...
    if not data_pairs:
        log.warning(f"No CA RMSD vs ligand RMSD pairs found for {split_name_key}, skipping correlation plot")
        return
    ...
    for metric_key, pairs in data_pairs.items():
        ...
        # Create scatter plot
        plt.scatter(x_vals, y_vals, alpha=0.7, color=color, edgecolor='black', s=70, zorder=3)
        
        # Add trend line
        try:
            z = np.polyfit(x_vals, y_vals, 1)
            p = np.poly1d(z)
            plt.plot(sorted(x_vals), p(sorted(x_vals)), "r--", alpha=0.8, linewidth=2, zorder=2)
        ...
        plt.title(f'C-alpha RMSD vs Ligand RMSD ({metric_key.capitalize()}) for {split_name_key}', 
        ...
        plt.axhline(y=2.0, color='#e67e22', linestyle='--', linewidth=1.5, label='2Å (ligand)', alpha=0.7, zorder=1)
        plt.axhline(y=5.0, color='#9b59b6', linestyle='-.', linewidth=1.5, label='5Å (ligand)', alpha=0.7, zorder=1)
        ...
        plt.gca().set_axisbelow(True)
        plt.gca().set_facecolor('#f9f9f9')
        plt.legend(loc='lower right', frameon=True, fontsize=11, bbox_to_anchor=(0.98, 0.02))
        plt.tight_layout()
        
        ts = datetime.now().strftime("%Y%m%d_%H%M%S")
        plot_file = os.path.join(plot_dir, f"ca_rmsd_vs_ligand_rmsd_{split_name_key}_{metric_key}_{ts}.png")
        plt.savefig(plot_file, dpi=300)
...
def save_comparison_table(all_benchmark_metrics: Dict, output_results_dir: str) -> None:
    ...
    header = [
        ...
        "CA_RMSD_Count", "MeanCA_RMSD(Å)", "MedianCA_RMSD(Å)", "AvgMatchedResidues", "AvgCoverage(%)"
    ]
    table_rows = [header]
    
    for split_key, metrics_data in all_benchmark_metrics.items():
        if not metrics_data or not metrics_data.get("per_method_stats"): continue
        ...
        for method_name, stats in metrics_data["per_method_stats"].items():
            row = [
                ...
                f"{ca_rmsd_avg_residues:.1f}" if isinstance(ca_rmsd_avg_residues, float) else 'N/A',
                f"{ca_rmsd_avg_coverage:.1f}" if isinstance(ca_rmsd_avg_coverage, float) else 'N/A'
            ]
    ...
    with open(md_file, "w") as f:
        ...
        for split_key, metrics_data in all_benchmark_metrics.items():
            ca_rmsd_stats = metrics_data.get("ca_rmsd_stats", {})
            correlation_metrics = [k for k in ca_rmsd_stats.keys() if k.startswith("correlation_with_")]
    ...
    for pdb_id, pdb_data in split_results_data.get("results", {}).items():
        ...
        if alignment_report_path and os.path.exists(alignment_report_path):
            ...
            try:
                    ...
                    for log_entry in alignment_data["detailed_logs"]:
                        if not log_entry.get("success", True):
                            ...
                            alignment_analysis["common_failure_stages"][stage] = \
                                alignment_analysis["common_failure_stages"].get(stage, 0) + 1
                ...
                alignment_analysis["validation_issues"].append(f"Failed to parse {alignment_report_path}: {e}")
...
def save_alignment_analysis_report(alignment_analysis: Dict, split_name: str, output_dir: str) -> Optional[str]:
    ...
    try:
        ...
        analysis_with_summary["analysis_summary"] = {
            ...
            "most_common_failure_stage": max(alignment_analysis["common_failure_stages"].items(), 
                                            key=lambda x: x[1])[0] if alignment_analysis["common_failure_stages"] else "None",
            "pdbs_with_missing_ca_rmsd": len(alignment_analysis["missing_ca_rmsd_patterns"]),
...
def main():
    ...
    parser.add_argument("--n-workers", type=int, default=suggested_config["n_workers"], help=f"Parallel PDB processing workers (auto: {suggested_config['n_workers']}). If >1, pipeline internal workers will be set to 1 to avoid oversubscription.")
    parser.add_argument("--n-conformers", type=int, default=200, help="Conformers per ligand")
    parser.add_argument("--template-knn", type=int, default=100, help="KNN for template selection")
    parser.add_argument("--similarity-threshold", type=float, default=None, help="Similarity threshold (overrides KNN)")
    parser.add_argument("--max-pdbs", type=int, default=None, help="Max PDBs per split (for testing)")
    parser.add_argument("--multiprocessing", action=argparse.BooleanOptionalAction, default=True, help="Enable/disable multiprocessing within true_mcs_pipeli6py (default: enabled)")
    parser.add_argument("--pipeline-timeout", type=int, default=600, help="Timeout per PDB in seconds (default: 10min)")
    parser.add_argument("--train-only", action="store_true"); parser.add_argument("--val-only", action="store_true"); parser.add_argument("--test-only", action="store_true")
    parser.add_argument("--log-level", type=str, choices=["DEBUG", "INFO", "WARNING", "ERROR"], default="INFO", help="Set the log level for the true_mcs_pipeline.py subprocess (default: INFO)")
    parser.add_argument("--ca-rmsd-threshold", type=float, default=10.0, help="Maximum C-alpha RMSD in Angstroms for protein filtering (default: 10.0)")
    
    # Add new embedding cache and batch processing arguments
    parser.add_argument("--use-embedding-cache", action=argparse.BooleanOptionalAction, default=True, help="Enable/disable embedding disk cache (default: enabled)")
    parser.add_argument("--embedding-cache-dir", type=str, default=None, help="Custom directory for embedding cache")
    parser.add_argument("--clear-embedding-cache", action="store_true", help="Clear the embedding cache before running")
    parser.add_argument("--batch-embedding", action=argparse.BooleanOptionalAction, default=True, help="Enable/disable batch embedding processing (default: enabled)")
    parser.add_argument("--max-batch-size", type=int, default=8, help="Maximum batch size for embedding processing (default: 8)")
    
    # Add histogram enhancement arguments
    parser.add_argument("--histogram-bins", type=int, default=50, help="Number of histogram bins for RMSD distribution plots (default: 50)")
    
    args = parser.parse_args()

    if not validate_paths(): log.error("Path validation failed. Exiting."); return
    allowed_pdbids_set = load_pdb_ids_from_file(ALLOWED_PDBIDS_FILE)
    if not allowed_pdbids_set: log.error(f"Allowed PDB IDs file {ALLOWED_PDBIDS_FILE} is empty. Cannot proceed."); return
    ...
    if args.clear_embedding_cache:
        if args.embedding_cache_dir:
            ...
            cache_dir = os.path.expanduser("~/.cache/templ/embeddings/esm2_t33_650M_UR50D")
            ...
            try:
                ...
                for filename in os.listdir(cache_dir):
                    if filename.endswith('.npz'):
                        ...
                        cleared_files += 1
                log.info(f"Cleared {cleared_files} files from embedding cache: {cache_dir}")
            except Exception as e:
    ...
    splits_data = {}
    for split_key, file_path in zip(["train", "val", "test"], [TRAIN_PDB_FILE, VAL_PDB_FILE, TEST_PDB_FILE]):
        ...
        log.info(f"{split_key.capitalize()}: {len(raw_pdbs)} raw -> {len(filtered_pdbs)} filtered PDBs")
    ...
    log.info("UniProt-based template exclusion between splits is NOT applied by this script.")
    log.info("Instead, PDB-based template pool restriction is used: validation and test sets can only use templates from the training set.")
    log.info("This is enforced by passing the training set PDB IDs file to true_mcs_pipeline.py with --enable-pdb-filtering.")


    all_benchmark_results = {"params": vars(args), "timestamp": BENCHMARK_TIMESTAMP, "split_info": {}, "results_data": {}, "metrics_summary": {}}
    ...
    if train_pdbs_for_template_pool:
        template_pool_file_path = os.path.join(RESULTS_DIR_CUSTOM, "train_set_template_pool.txt")
        ...
        log.info(f"Created template pool file from training set PDBs at: {template_pool_file_path}")
    else:
        log.warning("Training set is empty. Validation and Test set evaluations will not have a template pool from train.")


    for key, data in splits_data.items():
        all_benchmark_results["split_info"][f"custom_{key}"] = {
            "count_raw": len(data["raw"]), "count_filtered": len(data["filtered"])
    ...
    for split_key in ["train", "val", "test"]:
        if should_run_all or run_flags[split_key]:
            ...
            if split_key == "train":
                ...
                log.info(f"Skipping evaluation for '{split_key}' split as per revised plan.")
                # Add placeholder to results if needed or just skip
                all_benchmark_results["results_data"][f"custom_{split_key}"] = {"results": {}, "name": f"custom_{split_key}", "status": "skipped"}
                all_benchmark_results["metrics_summary"][f"custom_{split_key}"] = {"total": 0, "successful": 0, "status": "skipped"}
                ...
                if template_pool_file_path:
                    ...
                    log.warning(f"Cannot run {split_key} evaluation as training set template pool file is not available.")
                    all_benchmark_results["results_data"][f"custom_{split_key}"] = {"results": {}, "name": f"custom_{split_key}", "status": "skipped_no_template_pool"}
                    all_benchmark_results["metrics_summary"][f"custom_{split_key}"] = {"total": 0, "successful": 0, "status": "skipped_no_template_pool"}
                    continue

            split_run_data = evaluate_split(
                split_key, splits_data[split_key]["filtered"], 
                current_template_pool_file, # Pass the path to the train set PDB ID list file
                args.n_workers, args.n_conformers, args.template_knn, args.similarity_threshold,
                args.pipeline_timeout, args.multiprocessing, args.max_pdbs, args.log_level, # Pass the configured log level
                args.use_embedding_cache, args.embedding_cache_dir, # Pass embedding cache settings
                args.batch_embedding, args.max_batch_size, # Pass batch embedding settings
                args.ca_rmsd_threshold,  # Pass the CA RMSD threshold parameter
                BENCHMARK_TIMESTAMP  # Pass the global benchmark timestamp
            )
            all_benchmark_results["results_data"][f"custom_{split_key}"] = split_run_data
            ...
            plot_rmsd_distribution(split_run_data, f"custom_{split_key}", RESULTS_DIR_CUSTOM, args.histogram_bins)
            plot_ca_rmsd_distribution(split_run_data, f"custom_{split_key}", RESULTS_DIR_CUSTOM, args.histogram_bins)
            plot_ca_rmsd_vs_ligand_rmsd(split_run_data, f"custom_{split_key}", RESULTS_DIR_CUSTOM)
            ...
            if alignment_analysis["total_pdbs_processed"] > 0:
                print(f"Alignment tracking coverage: {alignment_analysis['pdbs_with_alignment_logs']}/{alignment_analysis['total_pdbs_processed']} PDbs")
                
                if alignment_analysis["missing_ca_rmsd_patterns"]:
                    print(f"⚠️  Found {len(alignment_analysis['missing_ca_rmsd_patterns'])} PDbs with missing CA RMSD values")
                    
                if alignment_analysis["validation_issues"]:
                    print(f"⚠️  {len(alignment_analysis['validation_issues'])} validation issues detected")
                    
                # Save detailed analysis report
                save_alignment_analysis_report(alignment_analysis, split_key, RESULTS_DIR_CUSTOM)
            else:
                print("No alignment tracking data found - consider running with enhanced tracking enabled")

    # Save overall results like polaris
    overall_results_file = os.path.join(RESULTS_DIR_CUSTOM, f"custom_benchmark_results_{BENCHMARK_TIMESTAMP}.json")
    ...
    if template_pool_file_path and os.path.exists(template_pool_file_path):
        try:
            ...
            log.warning(f"Error removing temporary template pool file {template_pool_file_path}: {e}")
            
    log.info("Benchmarking complete. Val and test splits were properly restricted to using only training set templates for KNN search.")

    # Print final stats about embedding cache if enabled
    if args.use_embedding_cache:
        try:
            cache_dir = args.embedding_cache_dir or os.path.expanduser("~/.cache/templ/embeddings/esm2_t33_650M_UR50D")
            if os.path.exists(cache_dir):
                files = [f for f in os.listdir(cache_dir) if f.endswith('.npz')]
                total_size = sum(os.path.getsize(os.path.join(cache_dir, f)) for f in files)
                log.info(f"Final embedding cache stats: {len(files)} entries ({total_size / (1024 * 1024):.2f} MB)")
        except Exception as e:
    ...
    for split_key in ["val", "test"]:
        if f"custom_{split_key}" in all_benchmark_results["results_data"]:
            ...
            if "experiment_output_dir" in split_data:
                log.info(f"  {split_key.capitalize()} poses: {split_data['experiment_output_dir']}")
...
class NpEncoder(json.JSONEncoder):
    def default(self, obj):
        ...
        if isinstance(obj, set): return list(obj)  # Convert sets to lists
