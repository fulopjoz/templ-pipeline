...
def check_python_version():
    ...
    if version >= (3, 9):
        ...
        print_status(
            f"Python 3.9+ required, found {version.major}.{version.minor}", "ERROR"
        )
        return False
...
def check_gpu_support():
    ...
    try:
        ...
        if gpu_available:
            ...
            print_status(
                f"✓ GPU available: {gpu_name} ({gpu_count} devices)", "SUCCESS"
            )
        else:
...
def check_cli_command():
    ...
    try:
        ...
        result = subprocess.run(
            ["templ", "--help"], capture_output=True, text=True, timeout=10
        )
