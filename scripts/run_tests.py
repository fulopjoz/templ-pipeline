...
class TestRunner:
    ...
    def __init__(self):
        ...
        self.presets = {
            ...
            'ci': ['--tb=short', '--cov-report=xml', '--durations=20', '--timeout=600', '--strict-markers', '--strict-config'],
            'parallel': ['-n', 'auto', '--dist=load', '--max-worker-restart=2', '--durations=20']
        }
    ...
    def build_command(self, config: str, preset: Optional[str] = None, 
                     parallel: bool = False, args: List[str] = None) -> List[str]:
        ...
        if parallel and preset != 'parallel':  # Don't duplicate if preset already includes parallel
    ...
    def run_tests(self, config: str, preset: Optional[str] = None,
                  parallel: bool = False, args: List[str] = None) -> int:
...
Examples:
  ...
  python run_tests.py --config performance --preset performance  # Performance tests
    ...
    parser.add_argument(
        '--preset',
        choices=runner.presets.keys(),
        help='Test preset to use'
    )
    parser.add_argument(
        '--parallel', 
        action='store_true',
