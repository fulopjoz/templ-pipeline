...
class CommandResult:
    ...
    def __init__(self, command: str, status: str, output: str = "", error: str = "", 
...
class HelpParser:
        ...
        for line in lines:
            ...
            if in_commands_section and line.startswith('  ') and not line.startswith('   '):
                ...
                if cmd_line:
                    ...
                    if command.replace('-', '').islower() and command.replace('-', '').isalpha():
    ...
    def extract_examples_from_help(self, help_text: str) -> List[str]:
        ...
        for line in lines:
            ...
            if stripped.startswith('Quick Examples:'):
                ...
                continue
            elif in_examples_section and stripped and not line.startswith(' ') and ':' in stripped:
    ...
    def _remove_example_prefixes(self, line: str) -> str:
        ...
        prefixes = ['Basic:', 'Template:', 'Parallel:', '$', '>', '•', '-', 'Example:', 'Usage:']
    ...
    def make_command_safe(self, command: str) -> List[str]:
        ...
        if any(arg in command for arg in ['--protein-file', '--ligand-file', '--embedding-file']):
    ...
    def check_python_environment(self) -> bool:
        ...
        try:
            ...
            if version.major >= 3 and version.minor >= 7:
                ...
                self.results['python_version'] = f"✗ Python {version.major}.{version.minor}.{version.micro} (requires >= 3.7)"
                return False
...
class CLITestRunner:
    ...
    def __init__(self, timeout: int = 30, verbose: bool = False, enable_discovery: bool = True):
        self.timeout = timeout
        self.verbose = verbose
        ...
        if self.enable_discovery and self.discovered_data:
            ...
            if discovered_commands:
                ...
                command_lists["discovered_module_help"] = [
                    f"python -m templ_pipeline.cli.main {cmd} --help" for cmd in discovered_commands
                ]
            ...
            if discovered_commands:
                command_lists["error_handling_tests"] = [
                    f"templ {cmd}" for cmd in discovered_commands[:3]  # Test missing args
                ]
    ...
    def run_command_tests(self):
        ...
        for category, commands in command_lists.items():
            ...
            for command in commands:
                ...
                if result.status == "PASS":
                    self.print_status(f"{command_display}: {result.status} ({result.duration:.2f}s)", "PASS")
                    self.analyze_output_behavior(category, result)
                elif result.status == "FAIL":
                    self.print_status(f"{command_display}: {result.status} - {result.details}", "FAIL")
                    if self.verbose and result.error:
                        print(f"   Error: {result.error}")
                elif result.status == "TIMEOUT":
                    self.print_status(f"{command_display}: {result.status} ({result.duration:.2f}s)", "WARN")
                else:
