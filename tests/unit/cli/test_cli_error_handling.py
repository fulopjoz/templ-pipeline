...
class TestInputErrors:
    ...
    def test_missing_required_arguments(self):
        ...
        for command, expected_arg in commands_and_args:
            ...
            assert (
                "required" in error_text or expected_arg in error_text
    ...
    def test_missing_dependency_error(self):
        ...
        with tempfile.NamedTemporaryFile(suffix=".pdb") as f:
            ...
            assert returncode in [
                ...
                1,
                2,
            ], "Should handle dependency issues appropriately"
    ...
    def test_disk_space_error_handling(self, mock_mkdtemp):
        ...
        with tempfile.NamedTemporaryFile(suffix=".pdb") as f:
            ...
            assert returncode in [
                ...
                1,
                2,
            ], "Should handle disk space issues appropriately"
...
class TestUserFeedback:
    ...
    def test_suggestions_for_common_errors(self):
        ...
        suggestion_indicators = ["usage", "help", "required", "example", "try"]

        has_suggestions = any(
            indicator in combined_output for indicator in suggestion_indicators
        )
    ...
    def test_proper_exit_codes(self):
        ...
        test_cases = [
            ...
            (
                ["embed", "--protein-file", "/nonexistent"],
                [1, 2],
                "Missing file should return 1 or 2",
            ),
        ]
        ...
        for command, expected_codes, description in test_cases:
            stdout, stderr, returncode = self.runner.capture_output(command)
            assert (
                returncode in expected_codes
            ), f"{description}: got {returncode}, expected {expected_codes}"
    ...
    def test_no_stack_traces_in_user_errors(self):
        ...
        for command in user_error_commands:
            ...
            assert (
                "Traceback" not in combined_output
            ), f"Command {command} should not show stack trace"
            assert (
                'File "' not in combined_output
            ), f"Command {command} should not show file traces"
    ...
    def test_helpful_command_suggestions(self):
        ...
        has_helpful_content = any(
            content in combined_output for content in helpful_content
        )
