...
from templ_pipeline.ui.services.pipeline_service import (
    PipelineService,
    run_pipeline_async,
)
...
class TestAsyncPipelineExecution:
    ...
    async def test_run_pipeline_async_basic(self):
        ...
        expected_poses = {
            "shape": (
                Mock(),
                {"shape_score": 0.8, "color_score": 0.7, "combo_score": 0.75},
            ),
            "color": (
                Mock(),
                {"shape_score": 0.7, "color_score": 0.9, "combo_score": 0.8},
            ),
            "combo": (
                Mock(),
                {"shape_score": 0.85, "color_score": 0.85, "combo_score": 0.85},
            ),
        }

        with patch(
            "templ_pipeline.ui.services.pipeline_service.PipelineService.run_pipeline",
            return_value=expected_poses,
        ) as mock_pipeline:
            # Run async pipeline
            ...
            # Note: async wrapper calls run_pipeline with molecule_data and protein_data dicts
    ...
    async def test_run_pipeline_async_with_exception(self):
        ...
        with patch(
            ...
            with pytest.raises(ValueError, match="Test error"):
                await run_pipeline_async(smiles="INVALID", protein_input="1iky")
    ...
    async def test_run_pipeline_async_concurrent(self):
        ...
        with patch(
            "templ_pipeline.ui.services.pipeline_service.PipelineService.run_pipeline",
            side_effect=lambda *args, **kwargs: {"poses": {}},
        ):
        ...
        with patch("concurrent.futures.ThreadPoolExecutor") as mock_executor_class:
            ...
            with patch("asyncio.get_event_loop") as mock_get_loop:
                ...
                with patch(
                    ...
                    assert (
                        call_args[0][0] == mock_executor
                    )  # First arg should be executor
                    assert (
                        call_args[0][1] == mock_pipeline
                    )  # Second arg should be mocked function

                    # Verify the function arguments were passed correctly
                    # Note: the async wrapper calls run_pipeline with dict arguments, not individual params
                    assert (
                        len(call_args[0]) >= 3
                    )  # Should have executor, function, and at least one argument
