...
def create_mock_pipeline_execution(success: bool = True) -> MagicMock:
    ...
    if success:
        mock_pipeline.run_full_pipeline.return_value = TestDataFactory.create_mock_pipeline_results(True)
        ...
        mock_pipeline.generate_poses.return_value = {'combo': (Mock(), {'combo_score': 0.8})}
    else:
        mock_pipeline.run_full_pipeline.side_effect = RuntimeError("Mock pipeline failure")
        mock_pipeline.generate_embedding.side_effect = ValueError("Mock embedding failure")
