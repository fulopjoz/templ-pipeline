...
class TestTimeSplit(unittest.TestCase):
    ...
    def test_splits_loading(self):
        ...
        if not self.splits_dir or not os.path.exists(self.splits_dir):
            self.skipTest(f"Could not find splits directory at any of the expected paths. Tried: {get_test_data_path('splits')}")
        
        if not self.embedding_path or not os.path.exists(self.embedding_path):
            self.skipTest(f"Could not find embedding file at any of the expected paths. Tried: {get_test_data_path('embeddings')}")
    ...
    def test_template_selection(self):
        ...
        try:
            ...
            self.assertIsNotNone(embedding_manager, "Embedding manager should be initialized")
        except Exception as e:
        ...
        self.assertGreater(embedding_count, 0, "Should find embeddings for some training PDBs")
        ...
        if test_pdbs:
            ...
            if query_embedding is not None:
                ...
                train_templates = [(pdb_id, sim) for pdb_id, sim in all_neighbors 
                                  if pdb_id in train_pdbs]
                
                # Log for debugging
                logger.info(f"Found {len(all_neighbors)} total neighbors, {len(train_templates)} in training set")
                ...
                for template_pdb, similarity in train_templates[:5]:
                    self.assertIn(template_pdb, train_pdbs,
