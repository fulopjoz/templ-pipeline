    ...
    def test_smiles_edge_cases(self):
        ...
        for case in edge_cases:
            try:
                ...
                assert isinstance(result, bool), f"Expected bool for {case}, got {type(result)}"
        ...
        for count in counts:
            try:
                if isinstance(count, int) and count > 0:
                    assert count <= 1000, f"Should limit molecule count to 1000, got {count}"
...
class TestErrorMessageConsistency:
    ...
    def test_error_message_format(self):
        ...
        for error in test_errors:
            assert re.match(combined_pattern, error), f"Error message doesn't match pattern: {error}"
    ...
    def test_chemistry_error_consistency(self):
        ...
        try:
            ...
            error_msg = str(e)
            assert "Invalid" in error_msg, f"Chemistry error format inconsistent: {error_msg}"
    ...
    def test_embedding_error_consistency(self):
        ...
        try:
            ...
            error_msg = str(e)
            assert "Invalid" in error_msg, f"Embedding error format inconsistent: {error_msg}"
...
class TestSecurityInputSanitization:
    ...
    def test_xss_prevention(self):
        ...
        for xss_input in xss_inputs:
            ...
            sanitized_full = re.sub(r"script.*?script", "", sanitized, flags=re.IGNORECASE)
            # Test passes if dangerous content is removed or reduced
            assert len(sanitized_full) <= len(xss_input), f"Sanitization did not reduce dangerous content: {sanitized_full}"
    ...
    def test_path_traversal_prevention(self):
        ...
        for traversal_input in traversal_inputs:
            ...
            clean_filename = safe_path.replace("..", "").replace("/", "").replace("\\", "")
            assert ".." not in clean_filename, f"Relative path markers not prevented: {clean_filename}"
            assert len(clean_filename) > 0, f"Filename completely removed: {clean_filename}"
    ...
    def test_command_injection_prevention(self):
        ...
        for command_input in command_inputs:
            ...
            for char in dangerous_chars:
                assert char not in sanitized, f"Command injection char not removed: {char} in {sanitized}"
...
class TestCrossPlatformCompatibility:
    ...
    def test_path_separator_handling(self):
        ...
        for test_path in test_paths:
            ...
            assert isinstance(normalized, Path), f"Path normalization failed for {test_path}"
            ...
            assert normalized.suffix in ['.sdf', ''] or '.sdf' in str_path, f"Path should contain expected file extension: {str_path}"
    ...
    def test_environment_variable_handling(self):
        ...
        for var in env_vars:
            ...
            if value is not None:
                ...
                assert isinstance(value, str), f"Environment variable {var} is not string: {type(value)}"
                # Should not be empty
