...
class TestCLIPerformance:
    ...
    def test_cli_import_speed(self):
        ...
        assert import_time < 0.1, f"CLI import took {import_time:.3f}s, should be < 0.1s"
    ...
    def test_no_heavy_imports_on_help(self):
        ...
        result = subprocess.run([
            ...
            "sys.argv = ['templ', '--help']; "
            "main(); "
            "heavy_modules = ['torch', 'transformers', 'rdkit']; "
            "loaded = [m for m in heavy_modules if any(m in mod for mod in sys.modules)]; "
            "exit(1 if loaded else 0)"
        ], capture_output=True, text=True)
        
        assert result.returncode == 0, f"Heavy modules were loaded during help: {result.stderr}"
    
    @pytest.mark.fast
    def test_lazy_loading_verification(self):
        """Test lazy loading is working correctly."""
        with patch.dict('sys.modules', {}, clear=False):
            # Mock heavy imports to fail
            with patch('builtins.__import__', side_effect=lambda name, *args, **kwargs: 
                      MagicMock() if name not in ['torch', 'transformers'] else ImportError()):
                
                # Help should still work
                stdout, stderr, returncode = self.runner.capture_output(["--help"])
                assert returncode == 0, f"Help failed with lazy loading: {stderr}"
                assert "TEMPL" in stdout, "Help output missing with lazy loading"
...
class TestSetupParserPerformance:
    ...
    def test_setup_parser_speed(self):
        ...
        assert setup_time < 0.1, f"Parser setup took {setup_time:.3f}s, should be < 0.1s"
