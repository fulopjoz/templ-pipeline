...
class TestEmbeddingManagerWithRealData(unittest.TestCase):
    ...
    def setUpClass(cls):
        ...
        if pdbbind_other_path and os.path.exists(os.path.join(pdbbind_other_path, cls.test_pdb_other)):
            cls.test_pdb_other_file = os.path.join(pdbbind_other_path, cls.test_pdb_other, f"{cls.test_pdb_other}_protein.pdb")
        else:
            cls.test_pdb_other_file = None
            
        if pdbbind_refined_path and os.path.exists(os.path.join(pdbbind_refined_path, cls.test_pdb_refined)):
            cls.test_pdb_refined_file = os.path.join(pdbbind_refined_path, cls.test_pdb_refined, f"{cls.test_pdb_refined}_protein.pdb")
        else:
            cls.test_pdb_refined_file = None
    ...
    def setUp(self):
        ...
        if not self.embedding_path or not os.path.exists(self.embedding_path):
            self.skipTest(f"Embedding file not found at any of the expected paths. Tried: {get_test_data_path('embeddings')}")
        if not hasattr(self, 'embedding_manager') or self.embedding_manager is None:
            self.skipTest("Embedding manager could not be initialized - check if embedding file exists and has the correct format")
    
    def test_load_embeddings(self):
        """Test loading embeddings from file."""
        # Verify embeddings were loaded
        self.assertTrue(len(self.embedding_manager.embedding_db) > 0, 
    ...
    def test_chain_data(self):
        ...
        self.assertTrue(len(self.embedding_manager.embedding_chain_data) > 0,
        ...
        self.assertIsInstance(chain_data, str, 
                             f"Chain data for {first_pdb} should be a string, got {type(chain_data)}")
        
        # Verify that chain data is correctly retrieved
        for pdb_id in list(self.embedding_manager.embedding_db.keys())[:5]:  # Check first 5 entries
    ...
    def test_find_neighbors(self):
        ...
        k = min(3, len(self.embedding_manager.embedding_db) - 1)  # At most 3 neighbors but not more than available
        ...
        self.assertEqual(len(neighbors), k, 
                        f"Expected {k} neighbors, got {len(neighbors)}")
        self.assertNotIn(query_pdb, neighbors, 
        ...
        self.assertEqual(len(neighbors_with_sim), k, 
                        f"Expected {k} neighbors with similarity, got {len(neighbors_with_sim)}")
        self.assertTrue(all(isinstance(item, tuple) and len(item) == 2 for item in neighbors_with_sim),
                       "Neighbors with similarity should be a list of (pdb_id, similarity) tuples")
        self.assertTrue(all(0 <= sim <= 1 for _, sim in neighbors_with_sim),
                       "Similarity scores should be between 0 and 1")
        
        # Verify neighbors are sorted by similarity (descending)
        for i in range(1, len(neighbors_with_sim)):
            self.assertGreaterEqual(neighbors_with_sim[i-1][1], neighbors_with_sim[i][1],
    ...
    def test_cache_operations(self):
        ...
        if not os.access(self.cache_dir, os.W_OK):
            self.skipTest(f"Cache directory {self.cache_dir} is not writable, skipping cache test")
        ...
        try:
            ...
            if is_cached:
                ...
                cached_embedding, cached_chains = self.embedding_manager._load_from_cache(test_pdb)
                self.assertIsNotNone(cached_embedding, f"Failed to load cached embedding for {test_pdb}")
                self.assertEqual(cached_embedding.shape, (1280,), 
                               f"Cached embedding for {test_pdb} should have shape (1280,), got {cached_embedding.shape}")
                
                # Check updated cache stats
                stats = self.embedding_manager.get_cache_stats()
                self.assertGreaterEqual(stats['count'], 1, "Cache should have at least one entry after caching")
        except Exception as e:
    ...
    def test_get_protein_sequence_from_real_pdb(self):
        ...
        if not self.test_pdb_refined_file or not os.path.exists(self.test_pdb_refined_file):
        ...
        self.assertIsNotNone(seq_other, f"Failed to extract protein sequence from {self.test_pdb_other_file}")
        self.assertGreater(len(seq_other), 20, 
                          f"Protein sequence from {self.test_pdb_other_file} should be longer than 20 residues, got {len(seq_other)}")
        self.assertGreaterEqual(len(chains_other), 1, 
        ...
        # Verify sequence was extracted
        self.assertIsNotNone(seq_refined, f"Failed to extract protein sequence from {self.test_pdb_refined_file}")
        self.assertGreater(len(seq_refined), 20, 
                          f"Protein sequence from {self.test_pdb_refined_file} should be longer than 20 residues, got {len(seq_refined)}")
        self.assertGreaterEqual(len(chains_refined), 1, 
...
class TestEmbeddingManager(unittest.TestCase):
    ...
    def setUp(self):
        ...
        try:
            np.savez(self.mock_embedding_path, 
    ...
    def test_basic_initialization(self):
        ...
        try:
            ...
            self.assertIsNotNone(manager, "EmbeddingManager should be initialized successfully")
            self.assertEqual(len(manager.embedding_db), 3, "Should load 3 embeddings from mock file")
            self.assertEqual(len(manager.embedding_chain_data), 3, "Should load 3 chain data entries from mock file")
        except Exception as e:
